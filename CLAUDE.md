...
## Knockdown behavior (⚙ button → `applyKnockdown()`, per-marker-segment + REF-vs-Standard ask build 86, per-marker-key code fallback build 88, forward-vs-nearest L/R conflict flagging build 89, REF-default + bulk-select review build 90)
...
- **Build 90 — the "Left untouched" section became an interactive list, not
  ...
  - **Delete selected** bulk-sets `.deleted=true` on exactly the checked
    points (ordinary delete semantics — archived as `Deleted` on export,
    never silently discarded) and removes them from the live list
    immediately; its own label shows a live count (`Delete selected (N)`).
  - `revertKnockdown`'s `skipped` entries now carry a `p` reference
    (`{id,hv,p}`) so this delete action has a real point to act on.
