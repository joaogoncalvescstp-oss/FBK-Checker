...
- **Bump the fruit version badge on EVERY push.** Use a *different* fruit each
  ...
  - History so I can tell what's live:
    ...
    | 93 | 🍉 WATERMELON | **fix a second real bug found on the SAME "-BUG" combined job file, using the same file rather than a fresh upload — owner: "FIX USIG SAME BUG FILE PIONT NUMBER 7670,7665 IS NOT USEIG REF PIONT."** Confirmed directly against the real file: point 7665 (`"RBCB RDRC"`, bare marker) has its own genuine REF shot right next to it (pt 7666), and point 7670 (`"RBCB R0PARK RDRC"` — `R0PARK` isn't a recognized `CURB_BOC` code, likely a field typo for `RPARK`, so this segment is ALSO effectively bare) has its own REF right next to it too (pt 7671) — but `applyKnockdown`'s `nearestRef` search silently found neither, because their true distances (**1.224 ft** and **1.234 ft**) sit just a few hundredths of a foot PAST the hardcoded `maxSearchRadius=1.2` cutoff, so both fell through to "no REF in range" and were left completely untouched instead of getting a REF-derived reveal. Rather than just nudging the threshold to cover these two specific points and calling it done, histogrammed the TRUE nearest-REF distance for every single `RBCB`/`RCFL` point in the whole 6581-point combined file (1243 points) to check whether 1.2 ft was ever the right number to begin with: the real data shows one coherent, smoothly-decaying cluster of REF standoff distances from about 0.3 ft up through **1.353 ft** (the crew's own real, consistent placement convention — hundreds of points densely packed in the 0.5-0.9 ft range, thinning out but still present through the low 1.2s and topping out at 1.353), then a genuine gap before the next, clearly separate value at 1.49 ft and a long unrelated tail climbing steadily into the hundreds of feet beyond that (REF shots that just happen to be nearby but belong to a different feature entirely). The old 1.2 ft cutoff cut straight through the MIDDLE of that one real cluster, not at its edge — silently excluding a real, correct REF match for any point unlucky enough to land in the 1.2-1.353 ft tail, points 7665/7670 included. Widened `maxSearchRadius` from `1.2` to `1.4` — comfortably covers the full real cluster (up to its true max, 1.353 ft) with margin on both sides, while stopping well short of the next, genuinely separate group starting at 1.49 ft, so it doesn't start pulling in unrelated REFs from other features. This is the ONLY place `maxSearchRadius` is referenced anywhere in the file (confirmed via grep) — a single, narrowly-scoped constant change, not a new code path. Verified: points 7665 and 7670 now both correctly bake a REF-derived reveal (`"RBCB H-0.5 V0.0 H-0.67 V-0.335 H-1.67 V-0.275 RDRC"` and `"RBCB H-0.5 V0.0 H-0.67 V-0.129 H-1.67 V-0.069 R0PARK RDRC"` — non-default V values, confirming the REF geometry was actually used, not the flat std template); re-ran the complete existing regression battery (`test_kd.js`, `test_kd2.js`, `test_kd3.js`, `test_kd_lines.js`, `test_kd_conflict.js`, `test_kd_flip.js`, `test_kd_bulk.js`, `test_noside_bulk.js`) — every one byte-identical to before, since none of those synthetic scenarios happen to place a REF in the newly-widened 1.2-1.4 ft gap; re-ran the existing single-site regression file (`job.fbk`, builds 86-90's own file) end-to-end — still the exact same **70 ambig + 3 sideConflict + 0 noSide** rows, confirming the wider radius doesn't manufacture any new false matches there either; re-ran the full combined bug file end-to-end — figure count unchanged (614), zero console errors, and the build-92 `noSide` row count rose from 211 to **218** (the 7 additional bare points, 7665/7670 included, that now correctly find their real nearby REF instead of being silently skipped for "no REF in range"). `index.html`'s inline script still parses clean (`node --check`). |
    | 94 | 🍇 GRAPE | **fix the offset lane line drawing a diagonal jog at a genuine mid-run H/V value change — owner: "FIX THE OFFET LINE WHEN NEW CALL OUT HV THE LINE SHULD DRARW FROM THE PREVAUS CALL OUT TO THAT POINT AND NEW CALL OUT SHOULD START LEAVING FROM THAT POINT."** Build 68 correctly made a curb-code RESTATEMENT with the SAME numeric value fully continuous (no false break just because `R612` re-expands to a fresh array each time), and builds 66/67 correctly made an explicit `SO` a real gap — but neither one ever asked "did the h/v VALUE itself actually change here?" A point that restates its own `H/V` (or a different curb code) with a genuinely DIFFERENT numeric value, with **no `SO` in between**, still had `cont[k]=true` (nothing stopped it) — so `offsetAt` mitered vertex k's corner using ONLY the new value on both adjacent legs, and the segment connecting it back to the previous (old-value) offset point silently jogged diagonally between two different offset distances, instead of the owner's expected "old lane ends here (old value), new lane starts here (new value)." Confirmed this is a common REAL pattern, not an edge case: scanning the real combined bug file (`TOPO_COMBINED_-_EDITED-BUG.fbk`, the same file builds 92/93 fixed) with `offsetPerCont` found **113** such genuine value-change points across the file. Fix: `offsetPerCont` now also returns `changed[k]` — true only when `stepsAt[k]`'s H/V values genuinely differ (by value, via a new `sameTemplate(a,b)` — never by object reference, learning from build 68's own reference-equality bug) from whatever template was active going into that vertex, and false for a same-value restatement (build 68, unaffected) or a resume-after-`SO` (build 66-68, unaffected, since `active` is `null` there so there's nothing to differ from). A new shared `laneOffsetArrays(all,per,cont,changed,L)` (used by both `drawOffsets` and `collectOffsetSegments`, replacing their previously-duplicated per-lane loops) computes, at a `changed[k]&&hasPrev` vertex, TWO points instead of one: `oldCap` — the OLD lane's own end-cap at k, computed with the PRIOR template's h/v and `offsetAt(...,true,false)` (a one-sided cap using the incoming k-1→k direction) — and `op[k]` itself forced to a NEW-lane start-cap, `offsetAt(...,false,hasNext)` (a one-sided cap using the outgoing k→k+1 direction, the new value). `contPrev[k]` is forced `false` at that vertex so the draw/snap loops break there instead of connecting; the old lane's own drawing loop gets one extra `lineTo(oldCap[k])` right before that break, so the old lane visibly reaches all the way to point k at its own (old) offset distance before the new lane picks up from the same point at its own (new) distance — no diagonal segment between the two. `collectOffsetSegments` (the OSNAP-facing rebuild) got the matching treatment so snap targets on the offset lines stay consistent with what's actually drawn. Verified: a synthetic 3-case Playwright check confirmed (A) a genuine value change with no SO produces `changed=[_,_,true,_]`, `contPrev` forced false at the change point, `oldCap` populated with the OLD value's end-cap and `op` with the NEW value's start-cap and no connecting segment between them; (B) a same-value redundant restatement stays fully continuous (`changed` all false, `oldCap` all null) — byte-identical to build 68; (C) a real `SO` gap-then-different-value-restart is NOT flagged `changed` (correctly handled by the pre-existing `cont`/gap mechanism instead, no double-handling). Re-ran the FULL existing Knockdown/Un-Knockdown regression battery (`test_kd.js`, `test_kd2.js`, `test_kd3.js`, `test_kd_lines.js`, `test_kd_conflict.js`, `test_kd_flip.js`, `test_kd_bulk.js`, `test_noside_bulk.js`, `test_unkd_bulk.js`, `test_unkd_clear_and_restore.js`) — every one byte-identical to before, since this change only touches offset LANE-LINE drawing, never `applyKnockdown` itself. Confirmed `collectOffsetSegments()` and a full `draw()` pass run with zero errors on both `job.fbk` (612 figures) and the real combined bug file (614 figures, 3087 offset segments); screenshotted a real occurrence (point 7276, a genuine H-value change mid-run) before and after the fix and confirmed the rendered output actually changed (not a no-op). `index.html`'s inline script still parses clean (`node --check`). |
    | 95 | 🍎 APPLE | **fix build 94's own fix leaving TWO real gaps — a lane that a callout drops or re-adds needs its own end-cap/fresh-start too, and a lane whose value is unchanged across that same callout must NOT break at all — owner sent a sketch + real point range (`7497` to `7502` on the bug file) showing "if there is call out making 3 offset line then next call out is making on 2 offset line[,] the 3rd line end or wait till the next call out with 3 offset and rejoin with that."** Build 94's `changed[k]` was computed once per VERTEX from the WHOLE template (`sameTemplate` — same length AND same values) and applied identically to every lane at that vertex. That correctly drew the "old lane ends here, new lane starts here" transition when a lane's own VALUE changed (build 94's original case), but had two real gaps once a callout changes the LANE COUNT instead (e.g. a real 3-step `H-1 V0.0 H-1.17 V-0.5 H-2.17 V-0.44` template dropping to a 2-step `H-1 V0.0 H-1.17 V-0.5` one — confirmed 9 real occurrences of exactly this shape in the bug file, on the `TBW`/`TTW` codes): (1) the disappearing 3rd lane had **no end-cap at all** — `laneOffsetArrays` checked `L<s.length` on the NEW (shorter) template FIRST and bailed straight to `null` for that lane, so it just stopped one vertex too early instead of drawing a clean perpendicular cap at the actual transition point; and (2) EVERY lane — including the 1st and 2nd, whose own h/v values are completely unchanged across that same callout — got a **spurious break** anyway, since `changed[k]` fires the instant the template's LENGTH differs, with no per-lane awareness that lanes 1/2's own numbers never moved. Fix: `laneOffsetArrays` no longer reads the coarse per-vertex `changed[k]` as its actual break decision — it now compares THIS LANE's own step (`prevStep` vs `curStep`, by `h`/`v` value) directly, computing a per-lane `laneChanged` that's true only when this exact lane's value genuinely differs, or the lane appears/disappears here; `changed[k]` is still consulted only as a cheap first gate (nothing to compare when the whole template didn't change at all). A disappearing lane now correctly gets `oldCap` (its own old-value end-cap, right at the transition vertex) even though it has no `op` there; a reappearing lane gets a fresh start-cap (`op`) with no `oldCap` (nothing to close, since it didn't exist a moment ago) — together this is the literal "3rd line end[s], or wait[s] till the next call out with 3 offset and rejoin[s] with that": a clean stop, a real gap through the reduced-lane stretch, and a fresh pickup once the callout restores the dropped lane, with the OTHER lanes (1st/2nd) staying completely unbroken the whole time since their own values never moved. Verified with a new synthetic 3→2→3-lane repro (`test_lane_drop.js`) mirroring the real `TBW` shape exactly: the persisting lanes (0 and 1) show `contPrev` staying `true` throughout with zero `oldCap` entries (confirming build 94's original spurious-break bug is gone); the dropped 3rd lane (lane 2) draws normally through the 3-lane stretch, gets a real `oldCap` exactly at the drop point (its own old value), goes `null` through the 2-lane stretch, then gets a fresh `op` start-cap exactly where the callout restores 3 lanes, with `contPrev` correctly `false` at both transition points (no jog either direction). Re-ran build 94's own 3-case regression (`test_offset_change.js`) unchanged — byte-identical, confirming the refined per-lane logic still produces the exact same result for the simple same-lane-count value-change case it was built for. Re-ran the complete existing Knockdown/Un-Knockdown regression battery (`test_kd.js`/`test_kd2.js`/`test_kd3.js`/`test_kd_lines.js`/`test_kd_conflict.js`/`test_kd_flip.js`/`test_kd_bulk.js`/`test_noside_bulk.js`/`test_unkd_bulk.js`/`test_unkd_clear_and_restore.js`) — every one byte-identical, since this change is still scoped entirely to offset lane-line drawing. Confirmed `collectOffsetSegments()`/a full `draw()` pass still run with zero errors and unchanged figure/segment counts on both `job.fbk` and the real combined bug file; found and dumped 9 real lane-count-change points in the bug file (`TBW`/`TTW` codes) confirming this is a real, not just synthetic, pattern. `index.html`'s inline script still parses clean (`node --check`). |
    | 96 | 🍌 BANANA | **fix build 95's own fix never actually DRAWING the disappearing lane's end-cap — the owner uploaded the real job file (`TOPO LEXINGTON-EDITED.fbk`) containing the exact point range they'd cited (7497-7502) and reported "It not behaving like I showd you."** Build 95 correctly computed the right DATA (`laneOffsetArrays` returns a populated `oldCap[k]` at the vertex where a lane drops from 3 to 2 offsets) but both places that actually consume it — `drawOffsets`'s lane-line loop and `collectOffsetSegments` — checked `op[k]` FIRST and bailed out (`started=false;continue` / `if(!o){prev=null;return;}`) before ever reaching the `oldCap[k]` check below it. At a vertex where a lane disappears, `op[k]` is correctly `null` (no new point for that lane there) — but that's exactly the vertex whose `oldCap[k]` needed to be drawn, and the early-exit skipped past it every time. Net effect: the disappearing 3rd lane simply stopped one real vertex too early (at 7495, not 7497) with no closing segment at all, instead of the clean end-cap right at the transition point the owner's sketch showed — a real, user-visible rendering bug build 95's own (numerically-correct) unit tests never caught, since they only checked the `laneOffsetArrays` return values, never that the drawing loops actually consumed `oldCap` correctly. Confirmed directly against the owner's real Lexington file: point 7497 (`"RBCB1 H-1 V0.0 H-1.17 V-0.5"`, dropping from 7443's 3-lane `H-1/H-1.17/H-2.17` template to just 2) is exactly this case — `oldCap` at 7497 for lane 2 computed correctly (`{E:561270.697,N:159061.145}`, ~2.17 ft from the base point, matching the dropped `H-2.17` lane's magnitude) but was never rendered. Fix: moved the `if(started&&oldCap[k]){...lineTo...}` check in `drawOffsets` to run BEFORE the `if(!op[k]){started=false;...}` early exit (so it fires even when this lane has no new point at `k`), and made the identical reordering in `collectOffsetSegments` (the `if(prev&&oldCap[k])` push now runs before the `if(!o){prev=null;return;}` bail). Both changes are pure reordering — no new conditions, no changed math — so every case that was already correct (same-lane-count value changes, redundant same-value restatements, real `SO` gaps) is untouched; only the "lane disappears entirely" case, which previously could never reach the `oldCap` branch at all, now does. Verified against the owner's real file: a wide screenshot centered on 7497 now shows the 3rd green offset line ending in a clean, isolated segment stopping exactly at the transition point (previously it just vanished one vertex early with no visible end at all); a matching screenshot at 7502 shows the 3rd lane starting fresh there, no diagonal connecting it back across the 2-lane gap — both screenshots match the owner's own sketch shape-for-shape. `collectOffsetSegments()` re-checked directly: it now emits a segment ending exactly at the same `{561270.70,159061.15}` point the draw path uses, confirming OSNAP and the visible canvas agree. Re-ran build 95's own synthetic 3→2→3-lane test (`test_lane_drop.js`) and build 94's 3-case test (`test_offset_change.js`) — both byte-identical (this fix is in the DRAW consumers, not the data-producing `laneOffsetArrays`, so their outputs are unaffected). Re-ran the complete existing Knockdown/Un-Knockdown regression battery (`test_kd.js`/`test_kd2.js`/`test_kd3.js`/`test_kd_lines.js`/`test_kd_conflict.js`/`test_kd_flip.js`/`test_kd_bulk.js`/`test_noside_bulk.js`/`test_unkd_bulk.js`/`test_unkd_clear_and_restore.js`) — every one byte-identical. Confirmed `collectOffsetSegments()`/a full `draw()` pass still run with zero errors on both `job.fbk` and the combined bug file; offset-segment counts rose slightly on both (job.fbk 3033→3077, bug file 3087→3090) — expected and correct, since these are exactly the previously-missing closing segments this fix now draws, not a sign of anything spurious. `index.html`'s inline script still parses clean (`node --check`). |
    | 97 | 🍒 CHERRY | **fix builds 94-96's own "genuine value change" detection over-firing on ordinary V-only variation — the owner: "Next fix on the offset the line are showing breaking and not flowing to the trantion like view build ago," clarified as "It happing on regular curb code and when applying the ref vertical ajustment."** Builds 94-96 treat ANY difference in a lane's own H/V numbers (`prevStep.h===curStep.h&&prevStep.v===curStep.v`, an exact equality on BOTH fields) as a genuine new callout that must end the old lane and start a fresh one — correct for the case build 94 was built around (an H distance that actually changes), but wrong for the far more common case of a V-only difference: a real curb's own vertical reveal legitimately varies from shot to shot along one continuous run at the SAME offset distance, whether it's crew-typed literal `H<v> V<v>` text (confirmed on the real combined bug file: point 3916→3917, `H0.5 V0.0 H0.67 V-0.5 H2.67 V-0.38` → `H0.5 V0.0 H0.67 V-0.02 H2.67 V0.1`, same 3 H values, V drifting up to 0.48 ft) or, even more so, Knockdown's own **REF-derived reveal** (`refKdTmpl`, this file's Knockdown section) — which by design computes a genuinely UNIQUE V from each point's own nearby REF geometry, so two adjacent REF-baked points on the very same curb code will almost always differ in V even though nothing about the callout itself changed. Scanned the real combined bug file end-to-end after running Knockdown (`applyKnockdown()`→`applyKdReview()` with every ambiguous row left at its default, exactly what a user clicking through the review modal without changing anything would produce): of every point where `offsetPerCont`'s coarse `changed[k]` gate fired, **231 of 345 (67%)** were this exact same-H/different-V shape — the SAME lane at the SAME offset distance, just a different (often REF-driven) reveal — and every single one of them was getting a spurious break (old lane ends, gap, fresh lane starts) instead of flowing through, exactly matching the owner's "breaking and not flowing to the transition" description and both halves of their clarification ("regular curb code" = the literal-H/V case; "ref vertical adjustment" = the REF-derived case). Fix: `laneOffsetArrays`'s `laneChanged` now compares ONLY `h` (`!(prevStep&&curStep&&prevStep.h===curStep.h)`, dropping the `&&prevStep.v===curStep.v` half entirely) — H is which physical offset distance/lane this is, so a real H change is still a genuine new lane (still breaks, unchanged); V is just that lane's own vertical reveal AT one point, and `offsetAt` already only uses this vertex's own `h`/`v` plus its real neighboring points' directions to compute the offset (never the PREVIOUS point's h/v), so letting a same-H/different-V vertex flow through the ordinary continuous branch (`hasPrev`/`hasNext` mitering, unchanged) produces exactly the right thing: the offset stays at the same distance from the base line, mitered normally at any real corner, with its Z simply following the new V — the same way the base line itself already flows through varying real Z without breaking. `changed[k]`/`sameTemplate` (the coarse whole-template gate feeding into this) were deliberately left untouched — they still fire on a V-only difference too, but that's fine, since the actual break decision now lives entirely in `laneOffsetArrays`'s per-lane, H-only check; a lane appearing/disappearing (build 95) is completely unaffected, since that's decided by `prevStep`/`curStep` being `null`, not by this comparison at all. Verified against the real combined bug file: re-ran the same real-Knockdown-then-scan sweep and checked, for every one of the 345 `changed[k]` events, whether each individual lane's `contPrev` at that vertex now matches the new H-only rule — **732 same-H lane-vertex instances now correctly flow continuously (`contPrev=true`), zero exceptions; 235 genuinely-different-H lane-vertex instances still correctly break (`contPrev=false`), zero exceptions** — a perfect split with no case landing on the wrong side either way. Re-ran build 96's own real Lexington-file verification (`test_lex_lane2.js`, the 7497→7502 lane-count-drop case) unchanged — `oldCap` still fires exactly at 7497, the 2-lane stretch through 7498-7500 stays correctly gapped, and 7502 still gets a correct fresh start — confirming this fix (a value comparison) doesn't touch the separate appear/disappear logic (a null-presence check) at all. Re-ran build 94's own 3-case regression (`test_offset_change.js`) and build 95's 3→2→3-lane regression (`test_lane_drop.js`) — both byte-identical, since neither test's synthetic "changed" scenario happened to be a same-H/different-V case (build 94's own test used an H change, `h:-1→h:-2`) — so this fix doesn't alter either test's already-verified outcome, it only corrects the real-world case neither test happened to cover. Re-ran the complete existing Knockdown/Un-Knockdown regression battery (`test_kd.js`/`test_kd2.js`/`test_kd3.js`/`test_kd_lines.js`/`test_kd_conflict.js`/`test_kd_flip.js`/`test_kd_bulk.js`/`test_noside_bulk.js`/`test_unkd_bulk.js`/`test_unkd_clear_and_restore.js`) — every one byte-identical, since this change is scoped entirely to offset lane-line continuity, never `applyKnockdown` itself. Confirmed `collectOffsetSegments()`/a full `draw()` pass and a real Knockdown-apply pass still run with zero errors on `job.fbk` (612 figures), the combined bug file (614 figures), and the real Lexington file (114 figures). `index.html`'s inline script still parses clean (`node --check`). |
    | 98 | 🍋 LEMON | **treat H the same as V — the owner, right after confirming build 97's V-only fix: "That's better. But you should treat the horizontal the same way you did the vertical. Let's try this again."** Build 97 stopped a same-H/different-V restatement from breaking the offset lane, but left a same-V/different-H restatement (or a different-H-and-different-V one) still forcing an old-lane-ends/new-lane-starts break — the owner's own reasoning for V (a real curb's own number legitimately varies point to point along one continuous run, and shouldn't be mistaken for "a new lane") applies just as directly to H: a real curb's offset distance can genuinely widen or narrow gradually shot to shot (a driveway apron easing in, a walk widening) without that being a brand-new, disconnected line either — and `offsetAt` already mitres purely off THIS vertex's own `h`/`v` plus its real neighboring points' directions, with no dependency on the PREVIOUS point's `h`, so letting a changing H flow through the same ordinary continuous branch build 97 already proved safe for V produces exactly the same right thing: the offset tracks the new distance smoothly, corner-mitred as usual, instead of jogging to a disconnected fresh start. Fix: `laneOffsetArrays`'s `laneChanged` dropped the `prevStep.h===curStep.h` comparison entirely — `const laneChanged=changed[k]&&hasPrev&&!(prevStep&&curStep);` — so a lane now only gets an end-cap/fresh-start when it genuinely **appears or disappears** at that vertex (one side has no step for this lane at all, build 95's lane-count-change case); as long as the lane exists on both sides, it flows continuously no matter what its own H or V numbers do, matching build 97's V-only precedent extended to both fields. Verified against the real combined bug file: re-ran the same real-Knockdown-then-scan sweep and checked every lane at every `changed[k]&&cont[k]` vertex — **977 lane-vertex instances where the lane exists on both sides now correctly flow continuously (`contPrev=true`), zero exceptions** (up from build 97's 732, since this now also covers the 235 different-H cases that build 97 still broke); **10 genuine appear/disappear instances still correctly break, zero exceptions**. Re-ran build 96's own real Lexington 7497→7502 lane-count-drop verification unchanged — byte-identical `op`/`contPrev`/`oldCap`, confirming this fix (relaxing the VALUE comparison further) has zero effect on the separate PRESENCE-based appear/disappear logic. Re-ran build 94's own 3-case regression (`test_offset_change.js`): case A (a synthetic `h:-1→h:-2` genuine distance change, previously build 94-97's own worked example for "this must break") now correctly flows continuously (`A_contPrev=[false,true,true,true]`, `A_oldCap` all null) — the deliberate, requested behavior change; cases B (same-value restatement) and C (real SO gap) are unaffected. Re-ran the complete existing Knockdown/Un-Knockdown regression battery (`test_kd.js`/`test_kd2.js`/`test_kd3.js`/`test_kd_lines.js`/`test_kd_conflict.js`/`test_kd_flip.js`/`test_kd_bulk.js`/`test_noside_bulk.js`/`test_unkd_bulk.js`/`test_unkd_clear_and_restore.js`) plus `test_lane_drop.js` — all exit clean, since this change is scoped entirely to offset lane-line continuity, never `applyKnockdown` itself. Confirmed `collectOffsetSegments()`/a full `draw()` pass and a real Knockdown-apply pass still run with zero errors on `job.fbk` (612 figures), the combined bug file (614 figures), and the real Lexington file (114 figures), figure counts unchanged from build 97. `index.html`'s inline script still parses clean (`node --check`). |
    | 99 | 🍇 GRAPE | **a lane a shorter callout drops now bridges STRAIGHT to wherever it next reappears, instead of capping and leaving a visible gap — the owner, with a concrete real example: "example 7497 to 7502 on the Lexington file... make it there the 3rd out most line piot 7497 connect to 7502 sipping 7498,7499,7500."** Builds 95/96 deliberately gave a dropped lane a clean perpendicular end-cap at the drop point, a real visual gap through the reduced-lane stretch, and a fresh start-cap where the callout restored it — the owner's OWN build-95 wording ("the 3rd line ends, or waits till the next call out with 3 offset and rejoins with that") is exactly what that shipped. This build is the owner asking for something genuinely different: no gap at all — the outermost (3rd) lane should visibly run straight from its last real point (7497) to its next real point (7502), skipping over 7498/7499/7500 where the callout only specified 2 offsets, the same way a real crew's curb line wouldn't show a hole just because a mid-run callout happened to omit that lane for a few shots. Fix: `laneOffsetArrays` now tracks `lastStep` (this lane's own most recently seen value, cleared only by a genuine SO or the very start of the run) — a vertex where the lane has no step of its own but `lastStep` exists gets a one-sided **cap** placed AT that real vertex using the OLD value (so the line still visibly reaches 7497's own position, not stopping one vertex short), tagged `real:false`; every vertex deeper into the drop (7498-7500) is left `null` with nothing to draw. Both `drawOffsets`'s lane-line loop and `collectOffsetSegments` now treat a `null` op as a **soft skip** — they only reset `started`/`prev` when `cont[k]` says a real `SO` actually broke it there; otherwise they skip straight through without resetting anything, so when the lane's own value resumes (7502), the draw path is still sitting at 7497's cap and a plain `lineTo` bridges the two directly, cutting straight across the gap — no cap-to-cap jog, no moveTo, no visible break. Build 94-98's whole `changed[]`/`oldCap` machinery (the "compare values, break on any change" model) is gone entirely, since it's no longer needed: `laneOffsetArrays` now only ever distinguishes "does this lane have a step here" (draw it, or bridge from wherever it last had one) from "did an actual SO stop it" (a real gap, unchanged, still driven purely by `cont[]`) — `offsetPerCont`'s `sameTemplate`/`changed[]` were dead code once nothing consumed them and were deleted along with `sameTemplate`. One real bug caught while implementing this: the BC..EC curve-fitting branch in `drawOffsets`'s lane loop used to gate on `op.slice(k-1,j+1).every(Boolean)` to decide whether every vertex in a curve span had a real per-vertex `h`/`v` to sample from — with drop-caps now able to make `op[k]` non-null even when `per[k]` doesn't actually carry this lane, that check would have read a cap's position as if it were a genuine step and then crashed trying to read `per[m][L].h` on a vertex that has no such entry; added a new `real[]` array (true only for a genuine step, false for a cap or a null) and switched that one check to `real.slice(k-1,j+1).every(Boolean)`, so a curve-fit only ever runs across a span where every vertex genuinely carries the lane — caught by a real crash in headless testing before it could ever reach the owner. Verified: the exact reported case (Lexington file, `RBCB1`, points 7493-7507) now shows lane 2's `op` as `[…,7495:real,7497:CAP(E=561270.697,N=159061.145 — the same value build 96 already ground-truthed),7498:null,7499:null,7500:null,7502:{E=561266.328,N=159074.287,contPrev:true},…]` — exactly "7497 connects to 7502, skipping 7498/7499/7500," with zero console errors. A new whole-file invariant check (`test_build99_verify.js`) confirms, across `job.fbk` (3433 drawable lane-points checked), `bugfile.fbk` (3384), and `lexington.fbk` (783): **`contPrev[k]` equals `cont[k]` (i.e. "was there really an SO") at every single drawable point, zero exceptions** — meaning a lane gap can now ONLY ever be forced by a genuine SO, never by a value change or a lane merely dropping in and out; and real bridged gaps (a lane null somewhere with non-null, no-SO points on both sides) are genuinely exercised on real data — 17 on `job.fbk`, 1 on `bugfile.fbk`, 26 on `lexington.fbk` — not just the synthetic case. Re-ran the complete existing Knockdown/Un-Knockdown regression battery (`test_kd.js`/`test_kd2.js`/`test_kd3.js`/`test_kd_lines.js`/`test_kd_conflict.js`/`test_kd_flip.js`/`test_kd_bulk.js`/`test_noside_bulk.js`/`test_unkd_bulk.js`/`test_unkd_clear_and_restore.js`) — all pass, unaffected, since this change is scoped entirely to offset lane-line continuity. `test_lane_drop.js` and `test_offset_change.js` (builds 95/94's own synthetic scenarios) were updated in place to the new `{op,contPrev,real}` return shape and the new expected bridging behavior — the only intentional assertion changes in either script, matching exactly what this build changes; `test_fix98_verify.js`'s build-98-specific "appear/disappear always breaks" assertion is superseded outright by this build's own `test_build99_verify.js`, which checks the equivalent (and now different) invariant directly against real data. Confirmed `collectOffsetSegments()`/a full `draw()` pass and a real Knockdown-apply pass still run with zero errors on `job.fbk` (612 figures), the combined bug file (614 figures), and the real Lexington file (114 figures); a real screenshot centered on the reported 7497-7502 stretch shows the outer green lane running distinctly from the two persisting inner lanes through that span, consistent with a direct bridge rather than a smooth multi-point trace. `index.html`'s inline script still parses clean (`node --check`). |
    | 100 | 🍊 ORANGE | **fix the outermost offset lane NOT curving through a real BC..EC span on the Lexington file — owner: "next I want you to fix that outer most line is not curving 7502 to 7512 on Lexington file."** Root cause was a direct side effect of build 99's own `real[]` eligibility gate on `drawOffsets`'s BC..EC curve-fit branch: real point 7502 is the `BC` of a genuine curved run (`"RBCB1 BC H-1 V0.0 H-1.17 V-0.5 H-2.17 V-0.44"`, 3 lanes) that curves through 7504/7507/7508/7510 to its `EC` at 7512 — but 7512's own callout (`"RBCB1 EC H-1 V0.0 H-1.17 V-0.5"`) only restates 2 lanes, not 3, since the crew's real EC callout simply doesn't re-state the outermost lane's value at the curve's own endpoint. For lane 2 (the outermost), that means the EC vertex has no `curStep` of its own — build 99's `laneOffsetArrays` correctly gives it a one-sided **cap** there (borrowing lane 2's last real value from 7510) so the line still visibly reaches 7512, but tags it `real:false` (a cap, not a genuine step) exactly as designed. The BC..EC curve-fit eligibility check, `real.slice(k-1,j+1).every(Boolean)`, was written to guard against sampling a curve through a true gap (a `null` — no position and no h/v at all) — but it couldn't tell that apart from a perfectly good cap (a real, already-computed position, just derived from a borrowed h/v instead of a fresh one), so it rejected the WHOLE 7502-7512 span the instant it saw 7512's cap, falling through to the plain per-vertex `lineTo` (straight/faceted) for that entire lane — exactly the "not curving" the owner reported, confirmed directly against the real file (`lane2_real` dumped `false` at 7512, `true` everywhere else in the span). Fix: `laneOffsetArrays` now also returns `hv[]` — the actual `{h,v}` that produced `op[k]`, whether from a genuine step or a cap's borrowed value — and the curve-fit eligibility check in `drawOffsets` switched from `real.slice(...).every(Boolean)` to `op.slice(...).every(Boolean)` (any drawable point — real or capped — is a valid curve endpoint; only a true `null` gap still disqualifies the span), with `hSub` built from `hv[m].h` instead of reaching into `per[m][L].h` directly (which is exactly what would have crashed on a capped vertex, the bug build 99 originally guarded against — `hv` sidesteps that by carrying the effective h/v value along with `op`, whether real or borrowed, so the crash-prone direct `per[]` lookup is gone entirely, not just newly guarded). Verified against the real Lexington file: the eligibility check for lane 2's 7502-7512 span now reads `true` (was `false`), `curvedOffsetPts` returns 40 smooth curve samples along the real fitted arc (was 0, falling to the straight per-point path), and a real headless-browser screenshot centered on the hairpin at 7502-7512 shows all 3 green offset lanes curving smoothly and concentrically with the magenta base line through the whole turn, matching the base curve's own shape with no facet. Re-ran the complete existing regression battery (`test_kd.js`/`test_kd2.js`/`test_kd3.js`/`test_kd_lines.js`/`test_kd_conflict.js`/`test_kd_flip.js`/`test_kd_bulk.js`/`test_noside_bulk.js`/`test_unkd_bulk.js`/`test_unkd_clear_and_restore.js`/`test_offset_change.js`/`test_lane_drop.js`/`test_build99_verify.js`) — every one still passes unchanged (`test_build99_verify.js`'s own invariant, `contPrev[k]===cont[k]` at every drawable point, still holds with zero violations and the same 26 real bridged gaps on the Lexington file), confirming this fix only widens which points a curve-fit may sample from, without touching the SO/gap/bridging behavior those tests check. `index.html`'s inline script still parses clean (`node --check`). |
    | 101 | 🍎 APPLE | **Knockdown's baked H/V must always land AFTER the line-control tokens (B/E/CLS/BC/OC/EC), and the curb DB code must always be removed regardless of REF or Standard — owner: "On the ref an standard tool when the ref is selected make sure to remove the curb db code out and when inserting hv code make it after BC,OS,EC,E,CLS as these contral the line .also double check that it that like that on the knockdown tool as well."** Checked `commitKnockdown` (the single function both the direct-apply path AND the `#kdReviewScrim` REF-vs-Standard/side-conflict/no-side review's Apply button both call — so there was only ever ONE place to fix, already answering "double check the knockdown tool too") against both halves of the report. The curb DB code half was already correct: a segment with its own code (`s.codeTokIdx!=null`) always got that exact token SPLICED OUT and replaced by the chosen template (`toks.splice(s.codeTokIdx,1,...tmplToks)`), whether `segTmpl(s)` returned the REF-derived reveal or the Standard cross-section — `refKdTmpl` returns only H/V numbers, never the code name, so no path could leave the literal db code text behind. The insertion-POSITION half was a real bug: a bare (no-own-code) segment's H/V always got spliced in right after the marker token (`toks.splice(s.start+1,0,...tmplToks)`), with no regard for where `BC`/`OC`/`EC`/`E`/`CLS` (or `B`) actually sat in the segment — a segment like `"RBCB B BC"` (REF-derived, no own code) would bake to `"RBCB H-0.5 V0.0 ... B BC"`, planting the offset BEFORE the very tokens that tell `figures()`/`strokeFigure` where the run begins and where its curve starts — the wrong side of the "line" boundary these tokens define, exactly what the owner flagged. Fix: a new `KD_LINE_CTRL` set (`B`, `BC`/`PC`, `OC`/`POC`, `EC`/`PT`, `E`, `CLS`/`CL` — the aliases parseDesc already treats identically) is scanned across the segment's own tokens (skipping the code token itself, which is never a line-control token) to find the LAST such token present; the H/V template is always inserted right after it, or right after the marker if the segment has none. The curb-code-removal path was also made explicit rather than relying on splice-replace: the code token is spliced OUT first, then the insertion index is corrected by −1 if the removed token sat before the computed insertion point — so removal and correct positioning both hold together even when the code token and a control token are in either order in the source data. **A real regression caught before shipping:** the first version of `KD_LINE_CTRL` (matching only the 5 tokens the owner named — `BC`/`OC`/`EC`/`E`/`CLS`, not `B`) broke the single most common real shape on file, `"RBCB B R612"` (a bare code with `B` stated first) — with no control token in the set, the H/V defaulted to landing right after the marker, shoving `B` all the way to the END: `"RBCB H-0.5 V0.0 ... B"`, moving the begin-token out of its normal leading position. `B` is exactly the same kind of line-control token as the other 5 (it's what `figures()` checks to open a run at all) — adding it to `KD_LINE_CTRL` fixed this and the owner confirmed the corrected requirement directly ("B E EC BC CLS be in front of hv code"). Verified: the exact multi-segment example from build 86's own docs (`"RBCB3 E R606 RBCB1 B BC R624"`) still bakes to the byte-identical, already-ground-truthed `"RBCB3 E H-0.5 V0.0 H-0.67 V-0.5 H-1.17 V-0.47 RBCB1 B BC H-0.5 V0.0 H-0.67 V-0.412 H-2.67 V-0.292"` (both `E` and `BC` were already positioned after their own segment's code token in every real case on file, so this fix is additive — it only changes output where a control token comes AFTER the marker but BEFORE where the bake would otherwise have landed); a new synthetic case (`"RBCB B BC"`, REF-derived, no own code) now correctly bakes to `"RBCB B BC H-0.5 V0.0 H-0.67 V-0.512 H-1.67 V-0.452"` (H/V after both `B` and `BC`, was before both, pre-fix); another synthetic case (`"RBCB B R624 BC"`, own code BEFORE a later `BC`, REF chosen in the review modal) now correctly bakes to `"RBCB B BC H-0.5 V0.0 H-0.67 V-0.512 H-2.67 V-0.392"` — `R624` is completely gone and the H/V lands after `BC`, not between `B`/`R624`'s old position and `BC`. Re-ran the complete existing Knockdown/Un-Knockdown regression battery (`test_kd.js`/`test_kd2.js`/`test_kd3.js`/`test_kd_lines.js`/`test_kd_conflict.js`/`test_kd_flip.js`/`test_kd_bulk.js`/`test_noside_bulk.js`/`test_unkd_bulk.js`/`test_unkd_clear_and_restore.js`) plus the unrelated offset-lane battery (`test_offset_change.js`/`test_lane_drop.js`/`test_build99_verify.js`, to confirm this commit-path change didn't disturb anything in the separately-tested drawing code) — every one byte-identical to its already-documented expected output. `index.html`'s inline script still parses clean (`node --check`). |
    | 102 | 🍌 BANANA | **Knockdown's own RBCB/RCFL segment could leak into a totally different figure code sharing the same line — owner: "Noticed other[] bug on knockdown and ref vs standard when there is multi[]code RBCB and RWLK on the same line[.] hv must [go] after RBCB code and B,E,BC,EC,OC,CLS[,] be careful there could [be] many code[s] on the same line[,] knockdown is only relevant to RBCB[,] RBCB.1-RBCB.9 code[.] basically just replace where the curb code occurs[,] R624[,] don't rearrange things."** Root cause: a marker's own SEGMENT (`start`/`end` in `applyKnockdown`) was bounded only by the next RBCB\d*/RCFL\d* marker — with no idea any OTHER figure code (RWLK, TBW, …) could sit on the same line, `end` fell all the way to the end of the description whenever no second RBCB/RCFL marker followed. Build 101's own control-token scan (added the same day, for the bare-no-own-code insertion case) then walked straight through that wrongly-unbounded range and could find `RWLK1`'s own `B`/`E` and land the baked H/V AFTER them — e.g. `"RBCB1 B BC RWLK1 B"` baked to `"RBCB1 B BC RWLK1 B H-…"`, planting the offset inside/after a figure Knockdown has no business touching at all. Two-part fix: (1) a new `curbSegEnd`/`isOtherCodeStart` correctly bounds a segment at the next GENUINE other marker — a non-RBCB/RCFL word counts as a real boundary only when it's immediately followed by an actual line-control token (`B`/`E`/`BC`/`EC`/`OC`/`CLS`/`PCC`/`PRC`/`CIR`/`SO` or their aliases), mirroring exactly how `parseDesc`'s own tokenizer would give it real flags right there — a bare descriptive word with nothing structural after it (the real, already-verified `"RDRC"`/`"R0PARK"` filler from builds 88/93) stays part of the CURRENT segment, unchanged; the segment's own curb-code family (`db`) is also excluded from ever counting as a boundary, so `R624` always stays in-segment. Both places `end` was computed (the pass-1 `markerCodes` index and the main per-point segment loop) now use this shared bound. (2) Per the owner's explicit "just replace where the curb code occurs, don't rearrange": build 101's own-code branch (`codeTokIdx!=null`) is simplified BACK to a plain in-place `toks.splice(codeTokIdx,1,...tmplToks)` — no control-token scanning or repositioning at all for this path, since every real, already-verified file has its own curb code positioned after `B`/`BC` by the time a crew types it, and scanning for a hypothetical rearrangement is exactly the mechanism that leaked into RWLK's tokens. The build-101 control-token-after-scan is now used ONLY for a truly bare segment (no code of its own — REF-derived), which still needs an insertion point of some kind and is now correctly bounded by the fixed `curbSegEnd`. Verified: the exact reported shape, `"RBCB1 B BC RWLK1 B"` (bare, REF-derived) now correctly bakes to `"RBCB1 B BC H-0.5 V0.0 H-0.67 V-0.512 H-1.67 V-0.452 RWLK1 B"` — H/V lands after `B`/`BC`, strictly before `RWLK1`, and `RWLK1 B` is completely untouched; the own-code variant, `"RBCB1 B BC R624 RWLK1 B"` (REF chosen in review), bakes to `"RBCB1 B BC H-0.5 V0.0 H-0.67 V-0.512 H-2.67 V-0.392 RWLK1 B"` — `R624` gone, replaced exactly where it sat, `RWLK1 B` untouched. Build 86's own multi-segment worked example (`"RBCB3 E R606 RBCB1 B BC R624"`) still bakes to the byte-identical, already-ground-truthed output. **A real regression caught before the fix above:** the first version of `isOtherCodeStart` treated ANY unrecognized capitalized word as a boundary regardless of what followed — this broke the real, already-verified build-88/93 `"RBCB RDRC R024"` case outright (`R024` fell OUTSIDE the shrunk segment entirely, `test_kd_conflict.js` crashed on a `null` `kdReviewCtx` and `test_kd_flip.js`'s `C1_hasPositiveH` flipped to `true` — the exact wrong-side flip build 88 had already fixed, regressed by this over-eager first attempt); the "must be immediately followed by a real control token" refinement fixed both while still catching the reported RWLK case. Re-ran the complete existing Knockdown/Un-Knockdown regression battery (`test_kd.js`/`test_kd2.js`/`test_kd3.js`/`test_kd_lines.js`/`test_kd_conflict.js`/`test_kd_flip.js`/`test_kd_bulk.js`/`test_noside_bulk.js`/`test_unkd_bulk.js`/`test_unkd_clear_and_restore.js`) plus build 101's own ordering test and the REF-hide round-trip test — every one byte-identical to its already-documented expected output. `index.html`'s inline script still parses clean (`node --check`). |
    | 103 | 🍓 STRAWBERRY | **removed the build-92 "no code anywhere for this marker — Left/Right?" popup entirely — owner: "I don't [know] what left and right review it should already [be] known... the knockdown is purely adjusting the vertical[,] final adjustments[,] [I] don't [know] why the[re's a] confusion here."** Asked to see a concrete real trigger before changing anything (project's own standing rule — never guess-fix without real data); pulled the exact case from a real combined multi-site job file (`bugfile.fbk`): point 3784, `"RBCB RDRC"`, part of a real 26-point curb line (`RBCB@116`, pts 3749→3986) where NOT ONE point ever states an actual code name (`L612`/`R612`) — several already have literal `H<v> V<v>` numbers typed directly instead of a code reference, everything else is bare. That's genuinely all 218 of this file's Knockdown-relevant points — every single one landed in the build-92/build-89 "no explicit code anywhere for this marker key" (`noSide`) bucket, which since build 92 asked the user to pick **Left**/**Right** per point (or in bulk) rather than silently guessing. After seeing the real data, the owner's actual instruction: **"if there isn't any code present, just skip the entire line and do not process it. The user will notice... that [it] wasn't drawn... and then... apply the code scan and run it again."** — i.e. don't ask, don't guess, don't bake anything: leave the segment completely untouched so its offset visibly fails to draw, which is the signal to go add a real code and re-run. Fix: the `!fwd&&!near` branch in `applyKnockdown` (previously flagging `noSide:true` and defaulting to a mirrored `R612` guess) is now a plain `return` — the segment is skipped before it's ever added to `segs`, so it's never in `pointWork`, never baked, never part of the review modal, and the point's description is byte-identical before and after Knockdown runs. Removed everything downstream that only ever existed to support the removed `kind:'noSide'` case as genuinely dead code: the `noSide` branches in `segTmpl`/`segActiveCode`/`segLogLabel`, the `noSide` Left/Right row markup in `renderKdReviewList`, the `ambigList` filter's `||seg.kind==='noSide'` clause, the **Set Left**/**Set Right** bulk buttons (HTML + `syncKdReviewToolbar` + `kdReviewSetChoice` + their `onclick` wiring) and openKdReview's hud text ("...or an unknown side"). The `sideConflict` (Carried forward vs Nearest declared — a genuine disagreement between two real candidates, not "nothing at all") and `ambig` (REF vs Standard) review kinds are completely untouched — they're a different situation (real, conflicting data) from `noSide` (no data at all) and the owner's objection was specifically to being asked about the latter. Verified: a synthetic lone bare `"RBCB RDRC"` + nearby REF with no code anywhere for that marker key now produces zero modal, zero PTS mutation, byte-identical description; a MIXED batch (one legit `ambig` row on a different point in the SAME Knockdown run, plus a `noSide`-shaped bare segment) still opens the review for the legit row, bakes it correctly on Apply, and leaves the noSide-shaped point completely untouched both before and after Apply — confirming the skip is scoped per-segment, not an all-or-nothing bail on the whole run; re-ran the real `bugfile.fbk` end-to-end and confirmed Knockdown now correctly hud's "no RBCB/RCFL curb codes to knock down" with zero points changed (all 218 of its points were exactly this case) and the concrete point shown to the owner, 3784, is still literally `"RBCB RDRC"` afterward; re-ran `job.fbk` (the file with real, legitimate ambiguity) end-to-end and got the byte-identical **70 ambig + 3 sideConflict + 0 noSide** rows and 324-point apply count already documented for it; re-ran `lexington.fbk` end-to-end (unaffected either way, since it never had a `noSide` case) — figure counts unchanged on all three (612/114/614), zero console errors. `index.html`'s inline script still parses clean (`node --check`). |
    | 104 | 🍒 CHERRY | **entering 3D orbit with a point selected now zooms to that point — owner: "when I have point select and I press to 3d orbit the that default to zoomed to piont and when nothing is select it behavior like normal like before."** `setMode('orbit')` (both the toolbar `#tOrbit` button and the `O` keyboard shortcut go through this one function) already called `fit()` on the 2D→3D transition (`is3D!==wasOrbit`) — that always framed the WHOLE model regardless of any current selection, discarding any point the user had just selected in 2D the instant they switched to orbit. Now, on that same transition, if a point is selected (`sel>=0`, and it isn't a deleted point) `fit()` still runs first (to get a sane baseline `orbit.s`/rotation for the model's own scale), then the pivot is re-targeted directly onto the selected point (`orbit.cx/cy/cz=p.E/N/Z` — the same "pivot = a real point" idea build 80's `retargetOrbitPivot` already established for orbit-dragging, just applied once up front here instead of needing a drag first), the zoom is bumped to at least `orbit.s=6` (the exact same absolute px-per-foot floor `zoomToPoint`'s 2D branch already uses, `view.s=Math.max(view.s,6)` — since `orbit.s` multiplies world coordinates the same way `view.s` does, the same constant carries over directly), and `ox/oy` are zeroed — since the pivot now equals the point's own coordinates, `P3` projects it to dead screen-center automatically with no extra offset math needed. The same gold flash-ring (`flash={i:sel,...}`) build 41's `zoomToPoint` already uses plays for visual feedback, matching every other "jump to a point" action in the app. **Nothing selected → completely unchanged**: `sel<0` skips the new branch entirely and falls through to the exact same `fit()` call as before, framing the whole model exactly like every orbit-entry did pre-this-build. A selected point that's since been deleted also falls back to the plain `fit()` (mirrors `zoomToPoint`'s own `if(!p)return` guard against acting on a stale/gone selection). Toggling `Orbit` again while already IN 3D (`is3D===wasOrbit`, no view transition happening) is untouched either way — it only ever hit the `else draw()` branch before and still does, so it can't re-snap the view out from under an owner who's already orbiting. Verified end-to-end through the real, running UI in a real headless browser (no sample `.fbk` job file is checked into this repo, so synthetic points spanning a large area were used, same approach as builds 80/83): with nothing selected, entering orbit reproduces `fit()`'s own whole-model centroid pivot exactly (byte-identical `orbit.cx/cy/cz` to computing it independently from `bounds()`); with a real point selected (including a non-centroid, off-center point, via an actual `#tOrbit` button click, not poked state) entering orbit sets the pivot to EXACTLY that point's own `E/N/Z`, `orbit.s` to the `6` floor, and the point's own `P3()` projection lands at EXACTLY screen center (sub-hundredth-pixel precision) — confirming the "zoomed to point" framing is real, not just a pivot change; re-clicking `Orbit` while already in 3D leaves the pivot completely unchanged (no unwanted re-snap while already orbiting); selecting a point and then marking it deleted before entering orbit correctly falls back to the plain whole-model `fit()`, matching the "acts normal" fallback. `index.html`'s inline script still parses clean (`node --check`). |
    | 105 | 🥝 KIWI | **new single-point REF knockdown tool — owner: "add single point ref knockdown[,] make user select RBCB point then REF point."** The whole-file `⚙ Knockdown` always finds its OWN REF automatically (nearest one within 1.4 ft, build 93) — with no way to override it for one specific point whose real REF sits out of that radius, or where more than one REF nearby means the wrong one gets auto-picked. New **🎯 REF Knockdown (1 pt)** toolbar button: click it, click the RBCB/RCFL point to knock down, then click its REF point directly on the canvas (works in 2D or 3D — plain point-picking via the same `pick()` used by `⌖ Go to point`/UGW pick, no geometric snap needed) — the chosen REF is used exactly as given, with the 1.4 ft search radius completely bypassed, since a manual pick is definitionally "the right one." Refactored `applyKnockdown`'s internals first, purely mechanically (verified byte-identical output before/after against the pre-refactor file on 3 scenarios, including build 86's own multi-segment worked example) — hoisted its previously-inline helpers (`KD_CTRL_TOKENS`/`isOtherCodeStart`/`curbSegEnd`/`refKdTmpl`, the marker-code index/`forwardMarkerCode`/`nearestMarkerCode` factory, and the whole per-point segment-resolution loop as `buildKdSegsForPointWork`) to module scope so the new tool reuses the EXACT same, already-verified (builds 86-103) own-code / bare-code-fallback / side-conflict logic instead of a second, drifting copy — only difference is `nearestRefFn`, a parameter now instead of a closure: `applyKnockdown` passes its usual distance-search version, the new `applySingleRefKnockdown(rbcbIdx,refIdx)` passes one that always returns the manually-picked REF regardless of distance. An own-coded segment still opens the existing `#kdReviewScrim` REF-vs-Standard review (pre-checked to REF, build 90's default) so the owner can see the exact number and back out or switch to Standard before anything is written; a bare (no-own-code) segment with no side conflict bakes immediately, since choosing this tool at all already answers "which REF" and there's nothing left to ask. Picking guards against the obvious mistakes: the first click must land on a point whose description carries an `RBCB\d*`/`RCFL\d*` marker (else a hud error, pick stays open for another try); the second click must land on an actual REF point (`isRefPoint`, same `/^REF\d*$/` first-token test `applyKnockdown` already uses) and can't be the same point as the first; **Esc** cancels a pick in progress at either step. A pulsing gold ring (`drawRefKdPick`, wired into both `draw2D`/`draw3D`) marks the picked RBCB point on screen while the REF pick is pending, the same "which point is this action about" pattern build 70's `drawUgwPickRing` already established. Verified two ways: (1) a real headless-browser regression comparing `applyKnockdown`'s output on 3 scenarios (the build-86 multi-segment example, the build-88/93 forward-vs-nearest RDRC shape, and a plain RCFL own-code case) against a byte-for-byte capture of the SAME scenarios run on the pre-refactor `index.html` — all 3 identical, zero console errors on either version, confirming the hoisting changed nothing about `applyKnockdown` itself; (2) real-UI, real-click end-to-end tests of the new tool in BOTH 2D and 3D (actual `#refKdBtn`/canvas/`#kdReviewApply` clicks, not poked state): an own-coded point with a REF placed 5 ft away (well outside the automatic tool's 1.4 ft radius — confirmed the automatic `⚙ Knockdown` correctly ignores it and bakes the flat std template instead) gets picked, opens the review pre-checked to REF, and Apply bakes the REF-derived reveal using that exact far REF; a bare point resolvable via the existing forward/nearest-code fallback bakes immediately with the manually-chosen REF's own v, no popup; clicking a non-RBCB point first is rejected with a hud message and the pick stays open; clicking a non-REF point or the same point again as the "REF" is rejected without consuming the pick; Esc cancels cleanly; the full sequence (button → RBCB point → REF point → Apply) reproduces correctly in 3D orbit view too, including the pulsing pick-ring. Zero console errors across every real-UI scenario. `index.html`'s inline script still parses clean (`node --check`). |
    | 106 | 🍑 PEACH | **new automatic REF1 outermost-vertical adjustment, folded into ⚙ Knockdown — owner: "ADD NEW FUNCTION ON THE KNOCKDOWN CURB[,] HAVE REF1 BE THE CODE TO ADJUST THE OUTER MOST VERTICAL OF THE CODE BY [REFERENCING] POINT WITH THE REF1 POINT[,] HAVE SEARCH [RADIUS] BE 3FT."** A DISTINCT tool from the existing REF/single-point-REF workflows above, triggered by the literal token `REF1` (not a generic `REF`/`REF2`/…) and scoped narrowly: it only ever touches the **OUTERMOST (last-stated) V value** of an already-baked RBCB H/V cross-section — never an earlier "mid" step — using a search radius of **3 ft**, wider than the ordinary 1.4 ft REF radius above. Clarified two open questions with the owner before implementing, given this project's long, costly history of "fix the fix" cycles on REF-related Knockdown logic (builds 86-103): (1) **automatic, folded into ⚙ Knockdown** — not a separate button, not limited to the build-105 manual single-point tool; (2) **REF1 only ever does this new thing** — a REF1-coded point is completely excluded from the ordinary REF-vs-Standard/REF-derived-reveal search (`findRefPoints()` now filters out any point whose token is exactly `REF1`), so the two mechanisms never overlap or double-adjust the same point off the same REF1 shot. `isRefPoint` (used by build 105's manual **🎯 REF Knockdown (1 pt)** tool to validate a hand-picked target) is unchanged — REF1 still validates there, since a manual pick is an explicit override with no search-radius ambiguity to worry about. New `applyRef1Adjustment()`: for every non-deleted, non-REF1 point carrying an `RBCB\d*` marker, finds the nearest `REF1` point within 3 ft (`nearestRef1`/`REF1_SEARCH_RADIUS=3`), computes `v=p.Z-ref.Z` (the same sign convention the ordinary REF-derived reveal already uses), and overwrites ONLY that marker segment's LAST `V<n>` token with `V`+`(-v-0.012).toFixed(3)` (the same baseline formula `refKdTmpl`'s own mid-step reveal already uses) — deliberately WITHOUT reapplying any `CURB_KD` per-code offset, since by the time a segment is already baked its original curb-code NAME is gone (replaced by numbers), so there's no code left to look a per-code kd delta up under; reapplying one would mean guessing which code it used to be, exactly the kind of guess this project's standing rule (build 40/47/49/103) argues against — disclosed as a deliberate design tradeoff, not an oversight. Idempotent (an exact-string comparison skips a token that's already correct, so re-running Knockdown twice never double-adjusts or logs a spurious edit) and multi-segment-safe (a point carrying more than one `RBCB` marker, e.g. the build-86 corner-point example, gets each marker's OWN outermost V adjusted independently, exactly mirroring how the rest of Knockdown already treats each marker segment on a point as its own unit). Scoped to RBCB (back-of-curb) only, matching this app's existing "REF-derived reveals are a back-of-curb-only concept" rule for RCFL (see the RCFL bullet below) — a nearby REF1 point never touches an RCFL segment. Runs at the tail of the single `commitKnockdown()` choke point both `applyKnockdown()`'s direct-apply path and the `#kdReviewScrim` review's Apply button funnel through (and, as a result, also automatically after build 105's single-point tool commits) — one wiring point, every Knockdown entry path gets it for free. **A real gap found and fixed while testing, not just a happy-path wire-up:** `applyKnockdown()` was found to bail out early — `hud('no RBCB/RCFL curb codes to knock down');return;` — the instant every RBCB/RCFL marker in the file was ALREADY fully baked (nothing fresh left to knock down), which meant `commitKnockdown()` (the only place the REF1 pass ran from) was never even reached — REF1 would have silently done nothing on a file that was knocked down in an earlier pass, exactly the realistic case a crew would actually use this for (shoot one extra REF1 point next to an already-curbed corner to fine-tune just the outer reveal, without wanting to touch anything else). Fixed by adding a dedicated branch right at that early-return point: if any `REF1` point exists in the file at all (a cheap precheck so the overwhelmingly common REF1-free case pays zero extra cost), `saveState()` then `applyRef1Adjustment()` run directly, and only fall through to the plain "nothing to knock down" hud if that pass, too, changed nothing. Verified end-to-end in a real headless browser: a regression scenario with NO REF1 anywhere (the build-86 multi-segment worked example, `"RBCB3 E R606 RBCB1 B BC R624"`, no REF/REF1 in the file) bakes byte-identical to before this build, with no REF1 mention in the hud; a REF1 point sitting within BOTH the old 1.4ft ordinary-REF radius and the new 3ft REF1 radius correctly skips the REF-vs-Standard review entirely (own code bakes flat Standard, no ambiguity) and THEN gets its outermost V adjusted by the REF1 pass immediately after — two independent mechanisms both firing, exactly as designed, not a conflict; an already-fully-baked 3-lane RBCB point with a REF1 within 3ft gets ONLY its last V token changed (the earlier `V0.0`/`V-0.5` steps confirmed byte-untouched) via the real `⚙ Knockdown` button click through the actual UI; a REF1 point beyond 3ft has zero effect and the point's description is byte-identical before/after; an RCFL point with a REF1 sitting right next to it is confirmed completely untouched (back-of-curb-only, as documented); running the adjustment twice in a row is confirmed idempotent (first pass adjusts 1 point, second pass adjusts 0); a multi-segment point (two separate `RBCB` markers in one description) gets each segment's own outermost V adjusted independently and correctly. Re-ran the existing build-105 regression suite (`test_single_ref_kd.js`/`test_single_ref_kd_3d.js`/`test_regression_compare.js`) and the orbit-zoom suite (build 104) — all pass unaffected, confirming this build's changes don't touch anything outside the Knockdown REF1 path. `index.html`'s inline script still parses clean (`node --check`). |
    | 107 | 🍇 GRAPE | **snap accuracy overhaul — owner: "CAN YOU IMPROVE THE ACCURACY OF THE SNAP AL[L] HAVE THE SNAP WORK ON ANY RENDERD LINES[.] INTERCETING LINE SNAP SHOULD ONLY SNAP WHEN TWO LINE INTERSECT EACH ON PLAN[.] I ALSO WANT TO ON CIRLCAS AND CURVES," linking MxCAD's own OSNAP reference doc.** Three real gaps in the OSNAP engine (`snapPoint`/`pickSegmentForInsert`/`collectSegments`, "Add Point (COGO) canvas pick + CAD snap" section above), all fixed: (1) **snap geometry was straight vertex-to-vertex chords, not the actual rendered line** — a BC..EC curve, an OC tangent-arc corner, and an RT/X/RECT jog chain all draw through extra computed points (`sampleCurve`/`fitTangentArc`/the jog-walk in `strokeFigureMain`) that `collectSegments()` never saw at all, so nearest/midpoint/intersection on a curve silently fell back to a straight chord across it — sometimes tens of feet off the true curve on a real compound curve. (2) **intersection was "apparent"** (build 12's own deliberate original design: two lines extended past their own ends until they cross) — the owner now wants the opposite: a hit only where the two REAL segments actually cross. (3) **no snap onto circles/curves specifically** — a `CIR` figure's or a standalone `⊙ CTR` circle's own center and 4 quadrant points (standard CAD "node"-like geometry, never on any chord) had no way to be targeted at all. Fix, in three parts: **(a)** extracted the exact point-generation walk `strokeFigureMain` already used to draw a figure into a new shared `figurePolylineFromVerts(f,vs)`/`figurePolyline(f)` — a circle's full 72-sample loop, each BC..EC sub-span's fitted arc/spline samples, an OC corner's tangent arc, and every RT/X/RECT synthetic jog point, returned as a plain world-space point array instead of being drawn straight to the canvas; `strokeFigureMain` itself was rewritten to just draw whatever that function returns, so there is now exactly ONE place that decides what a figure's line actually looks like, used by both rendering and the snap engine — no way for the two to drift apart. `collectSegments()` rebuilt on top of it: fine consecutive-pair segments per figure (tagged with a `src` id — see below), PLUS (new) the figure's own CPN/RPN tie lines (`f.links`), PLUS (new) any CPN/RPN tie whose origin isn't part of a figure at all (the same set `drawConnectLinks` draws), PLUS (new) every standalone `⊙ CTR` circle (`circles` array, tessellated at the exact same `radiusFt*0.3048` world radius `drawCircles` itself renders at, so snap matches the picture on screen even though that conversion factor is arguably a pre-existing separate oddity in `drawCircles` — deliberately left untouched, out of scope for a snap-accuracy pass). The pre-existing `collectOffsetSegments()` (curb cross-section offset lanes) is unchanged — still each lane's own straight vertex-to-vertex chords, a known, already-documented (build 27) tradeoff deliberately left alone here given `drawOffsets`'s own extremely fragile, 30-plus-build history (builds 66-100) of subtle correctness bugs in that exact function; duplicating its curve-fit logic for snap purposes was judged not worth the risk this pass. **(b)** `snapPoint`'s intersection stage now requires the crossing point to fall within BOTH segments' real parameter range (`X.t`/`X.u` both in `[-0.02,1.02]`, not unbounded) — a genuine crossing, never an extension past either line's own end; `pickSegmentForInsert`'s matching stage already bounded its OWN segment's `t` but never the OTHER line's `u` — now bounds both. Every segment from every source carries a `src` tag (a figure's own id, `id|off<lane>` for an offset lane, `id|link|<pt>` for a tie, `connect|<a>|<b>` for an unfigured tie, `circle|<i>` for a standalone circle) so the intersection scan can skip a pair sharing the SAME `src` — without this, two adjacent samples of one figure's own fine curve tessellation trivially "intersect" at the sample point they share, which would have manufactured a false self-intersection on literally every curve on screen; this is a NEW failure mode the fine-tessellation change in (a) would otherwise have introduced, caught and fixed in the same pass. **(c)** new `collectCircles()` returns every on-screen circle (`CIR` figures via `circle3`, standalone `⊙ CTR` circles) as `{E,N,Z,r,label}`; `snapPoint`'s endpoint tier (still gated by the same `endpoint` OSNAP checkbox — a center/quadrant point is exactly the same kind of precise "node" geometry a real point already is, so it didn't need a whole new toggle) also tests each circle's center and its 4 quadrant points (0°/90°/180°/270°, matching how the app's own circle tessellation already parameterizes a circle) against the cursor. `pickSegmentForInsert` deliberately does NOT gain a quadrant candidate — `insertPointOnLine` always re-derives the new point's E/N by interpolating between the two REAL vertices it's told to insert between, and a quadrant point isn't between any two real vertices, so there's no valid `(i1,i2,t)` to hand it (a first attempt tried anyway with a placeholder `t=0`, which silently inserted the new point at the WRONG location — caught before shipping, see below). `drawSnapMarker` gained two new marker shapes (a circle+cross for `center`, a diamond for `quadrant`) and the on-screen label/button text/comments were reworded from "apparent intersection" to plain "intersection" throughout. **A real bug caught and fixed before shipping, not just a clean first draft:** the initial quadrant addition to `pickSegmentForInsert`'s endpoint tier set `i1=i2=0,t=0` as a placeholder — since `insertPointOnLine(figId,i1,i2,t)` computes `newE=p1.E+(p2.E-p1.E)*t`, with `p1===p2===vs[0]` and `t=0` this collapses to exactly `vs[0]`'s own position, silently inserting the new point AT vertex 0 instead of at the clicked quadrant — found by re-reading the consuming function before considering the feature done, not by a test failure; removed entirely rather than patched, per the reasoning in (c) above. Verified: (1) a full render-output regression — captured every `ctx.moveTo`/`ctx.lineTo` screen-coordinate call for a scenario exercising all 6 branches (plain line, `CIR` circle, a `BC..EC` curve with an interior `PCC` split, an `OC` tangent-arc corner, an `RT`-chain-into-`RECT`, and a bare-`RECT` `closeL`) on this build vs. the pre-refactor committed file — **byte-identical, 173/173 draw calls**, confirming the `strokeFigureMain`/`figurePolyline` split changed nothing about what's actually drawn. (2) New real-browser feature tests: a circle's center and a quadrant point (deliberately placed away from all 3 real defining vertices) both resolve via `snapPoint` to the correct `type`/`E`/`N`; a standalone `⊙ CTR` circle likewise; two lines with a genuine crossing correctly resolve to `type:'intersection'` at the true crossing point; two SHORT lines whose infinite extensions would cross (the old "apparent" behavior) correctly resolve to `null`/non-intersection now that both segments are bounded; a click on an interior sample point of a `BC..EC` curve's own tessellation (deliberately NOT one of its real vertices) correctly resolves to `type:'nearest'`, never a false self-`'intersection'`; a `CPN`-tied point's tie line is confirmed present in `collectSegments()`'s output. (3) Real-UI (not poked-state) verification: opening **Add Point (COGO) → Pick on canvas**, hovering the real mouse over a circle's center, and clicking correctly fills the dialog's N/E fields with the circle's exact center coordinates via the genuine `startCogoPick`/pointermove/pointerdown pipeline. Re-ran the complete existing regression battery (`test_ref1.js`, `test_single_ref_kd.js`/`_3d.js`, `test_orbit_zoom.js`/`test_orbit_zoom2.js`) — all pass unaffected (the only non-passes are the same pre-existing, already-documented Google-Maps-network-block console noise these suites have always carried, unrelated to this change). `index.html`'s inline script still parses clean (`node --check`). |
    | 108 | 🍊 ORANGE | **fix "SNAP NOT WORKING ON OFFSET LINES" — the owner's terse follow-up right after build 107 shipped.** Build 107 deliberately left `collectOffsetSegments()` (the curb cross-section offset lane lines' own snap geometry) as straight vertex-to-vertex chords, disclosed at the time as a known tradeoff given `drawOffsets`'s 30+-build fragility history. Confirmed empirically this was the actual root cause, not a vague complaint: built a synthetic RBCB run with a real `BC..EC` curved offset lane and compared the OLD chord (straight BC-to-EC) against the actual rendered curve at its own midpoint — the chord sat **244px away** from the true curve at the tested zoom, far outside every snap tolerance (`TOL_NEAR=8`, `TOL_MID=11`, `TOL_X=14`), so hovering anywhere near the visibly-curved green offset line found nothing to snap to at all. A dead-straight offset lane (the common case) still worked (confirmed both before and after this fix), which is why the report reads as a blanket "not working" rather than "only broken on curves" — a user hovering a curved corner (a very common real shape for a curb run) gets zero snap and reasonably generalizes that offset-line snap is broken outright. Fix: extracted `drawOffsets`'s own per-lane point-walk (straight segments, `curvedOffsetPts` through a `BC..EC` span, `offsetTangentArc` through an `OC` corner, build 99's gap/bridge/cap rules for a lane a callout temporarily drops) into a new shared `offsetLaneChains(all,per,cont,bcAt,ecAt,ocAt,L)` — returns the lane's actual drawn geometry as world-point chains (a chain breaks only at a real `SO`, exactly like the draw loop) instead of calling `ctx.moveTo`/`lineTo` directly. `drawOffsets` now just walks whatever this returns and draws it (unchanged rendering, byte-verified); `collectOffsetSegments()` walks the SAME chains and builds fine snap segments from consecutive chain points instead of one chord per lane — the exact same single-source-of-truth pattern build 107 already used for `figurePolyline`/`strokeFigureMain`, just applied to the one function build 107 had explicitly left out. No changes to `offsetPerCont`/`laneOffsetArrays` themselves (the fragile build-94-100 gap/bridge/value-change machinery) — this only changes how their OUTPUT gets turned into a drawable/snappable shape, reusing that output identically in both places. Verified: (1) a full render-output regression (every `ctx.moveTo`/`ctx.lineTo` call, a scenario with a straight offset run, a `BC..EC` curved offset run, and an `OC` tangent-arc offset corner, pre- vs. post-fix) — **byte-identical, 142/142 draw calls**, confirming `drawOffsets`'s actual on-screen output is completely unchanged. (2) The same curved-lane scenario re-tested for snapping: hovering the real curve's own midpoint-tessellation sample now resolves via `snapPoint` to `type:'nearest'` at that exact point (sub-0.001ft error), where before the fix the chord-only geometry left it far outside tolerance; offset-segment count for a 3-figure scenario (straight + curve + OC corner) rose from 10 (old chords) to 29 (new tessellated segments) — expected, not a sign of anything spurious, matching exactly the extra curve/arc detail now being captured. (3) Straight-lane offset snapping (the pre-existing, already-working case) re-confirmed byte-behavior-identical — midpoint and nearest tiers still resolve correctly on a plain straight RBCB offset run. Re-ran the complete build-107 regression battery (`test_render_regression.js`, `test_snap_features.js` — circle center/quadrant, standalone circle, genuine intersection, rejected extended-only intersection, curve self-intersection avoidance, CPN tie segments) plus the existing Knockdown/REF1/orbit-zoom suites (`test_ref1.js`, `test_regression_compare.js`, `test_single_ref_kd.js`/`_3d.js`, `test_orbit_zoom.js`/`test_orbit_zoom2.js`) — every one passes unaffected (the two REF1 suites' own strict per-script error filter still flags the same pre-existing, already-documented `ERR_CERT_AUTHORITY_INVALID` console noise as a hard failure the way it always has, unrelated to this change — every actual assertion in both prints `true`). `index.html`'s inline script still parses clean (`node --check`). |
    | 109 | 🍋 LEMON | **fix a real Civil3D 2024 import failure — the owner uploaded their own job file (`TOPO_COMBINED_-_EDITED-SEP15.fbk`, exported by this app) and a screenshot of Civil3D's own import error: "File ... Line 490, Error: Unrecognizable input. (Improper number of parameters)" with Yes/No/Cancel to abort/ignore/ignore-all.** Line 490 of the uploaded file was exactly a build-84 `REF_HIDE` marker line (`"-- NEZ 4097 ... \"REF\""`) — confirming build 84's core assumption ("any FBK/CAD reader silently skips a line it doesn't recognize, the same way it skips a `Deleted <stamp>` archive line") was WRONG for a real Civil3D 2024 import: instead of skipping an unrecognized `-- `-prefixed line, Civil3D throws a BLOCKING per-line error dialog. This file has **407** such hidden REF lines (confirmed via grep) — meaning a real import of this file would pop that same dialog 407 times unless the owner clicked "Cancel" (ignore this and all subsequent) on the very first one, an extremely disruptive, easy-to-get-wrong workflow for a feature whose entire point was to be invisible/frictionless. Tried to find Civil3D's own documented FBK comment-line syntax to fix the marker itself rather than remove the feature, but every Autodesk domain (`help.autodesk.com`, `www.autodesk.com`, `forums.autodesk.com`, `docs.autodesk.com`) is blocked from this sandbox (the same standing limitation already documented under the RT/X/RECT vendor-doc section) and a web search turned up no independent source stating the exact accepted syntax — so, per this project's own standing rule against guessing at something the data (or, here, the actual target software) doesn't confirm, the fix does NOT try a different unverified marker. Instead: **build 109 stops writing REF_HIDE entirely.** A pure REF shot now exports as a perfectly ordinary line — identical in shape to every one of the other ~7000 lines in this exact file that DID import fine per the report — sacrificing the "hidden from CAD" cosmetic goal (REF points will now show up as plain visible points in Civil3D) in exchange for an import that actually works, which is unambiguously the higher priority once "hidden" turned out to mean "breaks the import." `REF_HIDE`/`deHide()`/`refHidden` are all KEPT on the READ side (`parse()`) so a job file exported by an OLDER build (84-108) — like the one the owner just uploaded — still loads correctly; `isPureRefDesc()` is deleted as now-genuinely-dead code (its only two call sites, both on the WRITE side, are gone). `exportFBK`'s step 2d is flipped from "hide a pure REF, un-hide a now-multi-code one" to a pure **self-heal**: it now only ever STRIPS a leftover `REF_HIDE` marker off any REF point that still has one from an older export (never adds one), so simply re-opening and re-exporting an already-broken job file fixes it — no separate "unhide" tool needed, and no user action beyond a normal export. `finalLine()` (used to rebuild an edited point's line, e.g. a Rod HT correction) got the matching simplification: it now only ever strips a leftover marker when present, never restores one. The step-3b end-of-file safety-net dump for standalone/COGO REF points was simplified the same way (always plain `NEZ`, no hide branch). Verified end-to-end against the owner's OWN real uploaded file, not a synthetic reproduction: parsing it directly confirmed 439 total REF points, 407 of them (exactly matching the grep count) flagged `refHidden:true` on load; exporting it through the real `exportFBK()` pipeline (Blob intercepted directly, not a real download) produced **zero** lines starting with `"-- "` anywhere in the 7502-line output; re-parsing that exported output confirmed all 439 REF points survive with `refHidden:false` on every one (self-healed) and the figure count unchanged at 618 both before and after, zero console errors. A separate synthetic test confirmed the forward-going behavior on a FRESH (never-hidden) file: a plain pure-REF shot (`"REF"`) now exports as an ordinary `F1 VA ... "REF"` line with no marker at all, and a multi-code REF shot (`"REF RCFL1 B"`, already always left untouched) is unaffected. Re-ran the complete existing regression battery (`test_ref1.js`, `test_regression_compare.js`, `test_single_ref_kd.js`/`_3d.js`, `test_orbit_zoom.js`/`test_orbit_zoom2.js`, plus build 107/108's own `test_render_regression.js`/`test_snap_features.js`/`test_offset_render_regression.js`) — every one passes unaffected (none of this touches Knockdown/REF1/snap/orbit logic at all, only how a REF line is written on export). `index.html`'s inline script still parses clean (`node --check`). **Open item the owner should confirm:** whether Civil3D's own "Deleted `<stamp>`" convention (the OTHER marker this project has relied on since early builds, for archiving a deleted point) also throws this same error — the uploaded screenshot only showed the FIRST error encountered (line 490, a REF_HIDE line), so it's not yet confirmed whether later dialogs (for other `-- ` lines, or possibly `Deleted` lines too) also appeared. If the owner reports `Deleted` lines ALSO throw this error, that would need the same kind of fix — flag it if so, rather than assuming it's fine by analogy alone (the same assumption that turned out to be wrong for `REF_HIDE`). |
    | 110 | 🍉 WATERMELON | **fix `finalLine()` silently "un-reversing" every genuine Face-2 shot's HA/ZA on export — the owner had a QC pass done on two real job files (`TOPO_PASCAL-EDITED.fbk` vs. a combined `TOPO_COMBINED_-_EDITED-SEP15.fbk`) that found 7 points (11331/11399/11428/11461/11465/11467/11471) with different HA/VA between the two, both still labeled `F2 VA` in both files, confirmed as an EXACT reciprocal-face transformation (HA shifted precisely 180.00000°, VA transformed to exactly 360°−VA) — and asked to look at every area in the app where an angle on a shot could get messed up this way.** Root cause, confirmed with a standalone numeric trace before touching any code: `parse()` (and `applyRawLine()`, the raw-FBK-code-editor path) both correctly flip a genuine Face-2 reading — `if(t[0]==='F2'&&za>180){za=360-za;ha=(ha+180)%360;}` — into its Face-1-equivalent form the moment the file loads, and `p.ha`/`p.za` are stored INTERNALLY in that corrected form from then on (the Inspector's own HA/ZA fields, `deg2dms(p.ha)`/`deg2dms(p.za)`, deliberately show this same corrected value for editing — that part was always fine). But `finalLine()` — the single function behind BOTH the "Review/edit FBK code" line preview and the real file export — took a completely different, buggy shortcut for every `F1`/`F2 VA` line: it always wrote `t[3]=deg2dms(p.ha)` and `t[5]=deg2dms(p.za)` straight from those already-corrected internal values, while leaving `t[0]` (the original `F1`/`F2` face token) untouched from the raw source line. For an ordinary Face-1 shot this is harmless (F1 never gets flipped, so `p.ha`/`p.za` already equal the raw values). For a genuine Face-2 shot with a raw ZA over 180°, though, this silently reversed the parse-time correction on **every single export**, of **every** untouched Face-2 point in the file — not just the 7 the owner's QC pass happened to diff — because it wrote the CORRECTED angle back out under the UNCHANGED `F2` label, producing exactly the "HA+180°, ZA=360−ZA vs. the original" signature the QC pass found. Traced this numerically first (a raw `F2 VA` shot with HA 45°/ZA 250.5432° parses to the corrected 225°/110°, and the old `finalLine` exported it right back out as `F2 VA ... 225 ... 110` — byte-for-byte the same transformation the owner's report described) before writing any fix, confirming this is the actual mechanism, not a guess. This matters for real use even though re-loading the app's own export back into the app itself computes the same N/E/Z either way (the correction is its own inverse, so the app's `za>180` gate never double-fires on its own re-parse) — the danger is any OTHER FBK-reading software (Civil3D, the original data collector, or a QC diff like the one the owner ran) that expects `F2` to mean "still needs the standard correction": it would see an already-corrected, no-longer->180° ZA under an unchanged `F2` label and either silently skip a correction it should apply, or (if it flips unconditionally rather than gating on ZA like this app does) undo the app's own correction and land back on the wrong, pre-correction angle — a real, silent position error in whatever consumes the exported file. Fix: `finalLine()` now re-derives, fresh from the untouched `RAW[p.srcLine]` text every time (never from any stored per-point flag), whether THIS line's own original face/ZA combination would have triggered the parse-time flip (`const flipped=t[0]==='F2'&&dms2deg(+t[5])>180`) — a fixed property of the source line's own original text, completely independent of whether the point has since been edited — and un-applies that same transformation to the CURRENT `p.ha`/`p.za` before serializing (`outHa=flipped?(p.ha+180)%360:p.ha; outZa=flipped?(360-p.za)%360:p.za`). An untouched point's corrected value un-flips right back to its original raw appearance (byte-for-byte numerically, modulo the same DMS-string round-trip precision `id`/`sd` already had); a genuinely edited Face-2 point (a rod-height fix, a coordinate edit, or a direct angle edit in the Inspector) still gets serialized correctly — the edit lands in the SAME raw-Face-2-looking convention (ZA still >180) the source line used, so it reduces to the right position whether re-parsed by this app or read by any other tool that follows the standard reciprocal-face convention. Verified with the REAL extracted `finalLine`/`dms2deg`/`deg2dms` source (not a reimplementation) against 4 cases: an untouched genuine Face-2 shot (raw ZA>180) now round-trips its HA/ZA back to the original raw values, where the old code exported the wrong, reversed pair; a plain `F1` shot is completely unaffected (never flips, matching before); an `F2`-labeled shot whose raw ZA was already ≤180 (never needed the flip in the first place) round-trips unchanged either way; and a genuinely EDITED Face-2 shot (Inspector ZA changed from the corrected 110° to 105°) exports as a proper raw-Face-2-looking line (ZA still >180) that re-parses back to exactly the edited 105°, not the original 110° and not a double-flip. Also grepped every other `deg2dms(p.ha)`/`deg2dms(p.za)` call site in the file to confirm `finalLine()` was the only place with this bug — the Inspector's own HA/ZA edit fields (correctly showing the corrected value for editing) and the `moveTo`/edit-log message (cosmetic only, never exported) were both already fine and untouched. `index.html`'s inline script still parses clean (`node --check`). |
    | 111 | 🥭 MANGO | **new ↓ Export DXF button** — the app could export FBK/CSV but never a drawing file, so getting the linework into CAD meant re-importing the FBK/CSV and re-tracing every curve/circle/curb offset by hand. `exportDXF()` writes a plain ASCII (R12-style) DXF with just an `ENTITIES` section — no `HEADER`/`TABLES` needed, since AutoCAD/Civil3D auto-creates any layer an entity references — X=Easting, Y=Northing, Z=Elevation throughout (the standard state-plane CAD convention, matching how a PNEZD point file already gets consumed downstream). Every non-deleted point gets a `POINT` entity (layer `POINTS`/`CONTROL-POINTS`) plus a `TEXT` label using `pointLabelText(p)` — the exact same string the 🏷 Labels toolbox already shows on screen, so the DXF labels match whatever the owner currently has configured (falls back to the bare point id if every label checkbox is off, so a point is never unlabeled in the DXF). Every figure's line is written via `figurePolyline(f)` — the SAME already-verified (build 107) function rendering and snap both use for a figure's actual tessellated geometry — as either a `CIRCLE` entity (when the figure IS a circle) or a 3D `POLYLINE`/`VERTEX`/`SEQEND` block (real per-vertex Z, so a curve's elevation profile survives, not just its plan shape) on a layer named `LINE-<code>`; CPN/RPN ties (both a figure's own `f.links` and any tie whose origin isn't part of a figure at all, mirroring `drawConnectLinks`'s own two-source logic) become `LINE` entities on layer `TIES`; standalone `⊙ CTR` circles become `CIRCLE` entities on layer `CIRCLES`. The curb offset cross-section lane lines are written the same disciplined way — by calling `offsetPerCont`/`offsetLaneChains` directly, the exact functions `drawOffsets`/`collectOffsetSegments` already use (build 94-100/108's hard-won gap/bridge/curve machinery) — as `POLYLINE`s on layer `OFFSET-<code>`, rather than re-deriving that geometry a third time and risking yet another subtle offset bug in a brand-new code path. Deliberately reused every existing, already-battle-tested geometry function instead of writing new tessellation/offset logic, given this project's own long history of exactly that class of bug. Verified end-to-end in a real headless browser (not a reimplementation) against a synthetic job exercising every entity type at once — a plain 2-vertex line, a `BC..EC` curve, a lone 3-shot `CIR` cluster, and an `RBCB1` curb run with a real `H/V` offset template through its own `BC..EC` span: the exported DXF starts with `SECTION`/`ENTITIES` and ends with `ENDSEC`/`EOF`; exactly one `POINT` and one `TEXT` per parsed point; the `CIR` cluster produces a `CIRCLE`; the straight line and the curve both produce a `POLYLINE` with a matching `SEQEND` and more than 2 `VERTEX` records for the curve (confirming real curve tessellation came through, not a straight chord); the `RBCB1` run's `H/V` template produces a real `offsetLaneChains` chain that shows up as an `OFFSET-RBCB1` polyline; zero `NaN`/`undefined` anywhere in the output; zero console errors beyond the pre-existing, already-documented map-tile network block. A second synthetic test confirmed a `CPN` tie between two figures exports as a `LINE` entity on the `TIES` layer. `index.html`'s inline script still parses clean (`node --check`). |
    | 112 | 🍎 APPLE | **added the City of Saint Paul logo to the top-left toolbar** — owner: add `https://www.governmentjobs.com/AgencyPages/stpaul/agencyImages/download/stpaul-logo-vertical-sky-tinted-blue.png` to the top-left corner. Added directly as an `<img class="brandLogo">` inside the existing `.brand` div (which already reads "Saint Paul · Public Works / FBK Canvas" — this app is the city's own internal tool, so its own city logo belongs there), switching `.brand` from a single vertical flex column to a horizontal row (`flex-direction:row;align-items:center;gap:8px`) with the logo on the left and the existing two text lines now wrapped in a plain `<div>` so they keep stacking vertically next to it. `.brandLogo{height:30px;width:auto}` sizes the logo to the toolbar's own height without distorting its aspect ratio. **Verification is genuinely partial, disclosed rather than assumed:** confirmed the HTML/CSS is structurally correct — `node --check` passes, the `.brand` flex layout renders with the logo and text side by side with no layout breakage, zero console errors — but this sandbox's own network policy could not actually fetch the image (`naturalWidth`/`naturalHeight` both came back `0`, the same "loaded, but failed" signature the Street View sections above document for Google's domains), so the ACTUAL logo artwork — whether it looks right at 30px, whether its own aspect ratio/whitespace looks good next to the existing text — is unverified from here, the same standing caveat every external-asset feature in this app carries. The owner should confirm the real image renders and looks right in their own browser. |
  - Suggested next fruits to rotate through:
    🍌 BANANA, 🍇 GRAPE,
    🍊 ORANGE, 🍓 STRAWBERRY.

## Knockdown behavior (⚙ button → `applyKnockdown()`, per-marker-segment + REF-vs-Standard ask build 86, per-marker-key code fallback build 88, forward-vs-nearest L/R conflict flagging build 89, REF-default + bulk-select review build 90, unknown-side ask build 92 REMOVED build 103 — see below, control-token bake ordering build 101, multi-code segment isolation build 102, single-point manual REF pick build 105, automatic REF1 outermost-vertical-only adjustment build 106)

- **REF1 — automatic outermost-vertical-only adjustment, 3ft radius** (build
  106). A DISTINCT marker from plain `REF`/`REF2`/…: a point coded exactly
  `REF1` never counts as an ordinary REF source (`findRefPoints()` excludes
  it, so it can't trigger or feed the REF-vs-Standard review above) — it
  exists solely to correct the **outermost (last-stated) V** of an
  already-baked RBCB H/V cross-section within **3 ft** (`REF1_SEARCH_RADIUS`
  — wider than the ordinary 1.4 ft REF radius, and a completely independent
  constant from it). Runs automatically every time Knockdown commits — no
  separate button, no per-point ask — via `applyRef1Adjustment()`, called at
  the tail of `commitKnockdown()` (so it fires after the direct-apply path,
  the `#kdReviewScrim` review's Apply, AND build 105's single-point tool
  alike) and also directly from `applyKnockdown()` itself when there's
  nothing fresh left to bake but a REF1 point still needs to adjust an
  already-baked point. Uses the same `v=p.Z-ref.Z` sign convention and
  baseline formula as the ordinary REF-derived reveal, but never reapplies a
  `CURB_KD` per-code offset (the original code name is gone once a segment
  is baked, so there's nothing to look one up under — a deliberate choice,
  not an oversight). Idempotent, multi-segment-safe, and RCFL segments are
  never touched (back-of-curb only, matching the RCFL rule below). See the
  build 106 entry above for the full writeup and verification.

- **🎯 REF Knockdown (1 pt)** (build 105) is a separate, manual sibling of
  `⚙ Knockdown` for exactly one point: click the tool, click the RBCB/RCFL
  point, then click its REF point directly on the canvas (2D or 3D) —
  `applySingleRefKnockdown(rbcbIdx,refIdx)` bakes that point using the
  manually-picked REF exactly as given, with the automatic 1.4 ft search
  radius below completely bypassed. Use it when a point's real REF sits
  outside that radius, or when more than one REF nearby means the automatic
  nearest-REF search would pick the wrong one. Shares the exact same
  own-code / bare-code-fallback / side-conflict resolution logic as the
  whole-file tool (`buildKdSegsForPointWork`, factored out of
  `applyKnockdown` for this reuse) and the same `#kdReviewScrim` review
  modal for an own-coded segment (pre-checked to REF) — see the build 105
  entry above for the full writeup.
- Knockdown processes each **RBCB\d\*/RCFL\d\* marker in a point's description
  ...
  to the next RBCB\d*/RCFL\d* marker, OR the next genuinely different figure
  code sharing the same line, whichever comes first — see build 102 below)
  gets its own code lookup, its own "already baked" check, and its own
  bake/ask decision. See the build
...
- **RBCB** (rod on back of curb) → uses the **Back-of-Curb DB** (`CURB_BOC`).
  - A segment with its own curb code AND a REF point within range (the
    search radius — **1.4 ft since build 93**, widened from the original
    1.2 ft after a real combined job file showed points landing just past
    the old cutoff despite having a genuine nearby REF; see the build 93
    entry above for the full histogram-based justification) now **asks
    the user** (build 86) — a
  ...
  - **Build 92 introduced a THIRD case, distinct from sideConflict: nothing
    declared for this marker key ANYWHERE in the file, not merely
    disagreeing** — `forwardMarkerCode`/`nearestMarkerCode` both returning
    `null`. Build 92 flagged this as `kind:'noSide'` and asked the user to
    pick **Left**/**Right**, defaulting to Right (the old silent
    behavior). **Build 103 removed this entirely, per the owner's explicit
    request.** The owner's own framing: the horizontal (L/R) side should
    already be settled by the time Knockdown runs — Knockdown's job is
    purely the vertical (REF) adjustment — so being asked to pick a side at
    all was the confusion, not which default it landed on. Shown a real
    concrete trigger (a combined job file where NOT ONE point of a 26-point
    curb run ever states an actual code name — see build 103's own entry
    above), the owner's instruction was explicit: **"if there isn't any
    code present, just skip the entire line and do not process it... the
    user will notice... it wasn't drawn... then apply the code scan and run
    it again."** So a segment with no code anywhere for its marker key
    (`!fwd&&!near`) is now simply **skipped** — left byte-identical,
    excluded from `pointWork` entirely, never baked, never part of any
    review. There's no more `kind:'noSide'`, no Left/Right radio, and no
    **Set Left**/**Set Right** bulk buttons — `segTmpl`/`segActiveCode`/
    `segLogLabel`/`renderKdReviewList` only ever see `'ambig'`,
    `'sideConflict'`, and `'resolved'` now. The visible symptom of a skip
    is simply that segment's curb offset never gets drawn (no H/V was ever
    baked into it) — the owner's own intended workflow is to notice that,
    add a real code somewhere for that marker key (on this point or any
    other sharing the same key), and re-run Knockdown, at which point it
    resolves normally via the existing forward/nearest/own-code paths.
    `sideConflict` (a genuine disagreement between two real, non-null
    candidates) is a completely different situation and is untouched by
    this — it's still flagged and asked about exactly as build 89 left it.
...
## REF-hide on export — REVERTED build 109 (`REF_HIDE`, `exportFBK` step 2d, `parse()`'s `deHide()`, build 84, removed build 109)

- **Builds 84-108 commented a pure REF shot out of the export** (prefixed its
  line with `REF_HIDE`, `'-- '`) on the theory that any FBK/CAD reader only
  recognizes specific keyword lines and silently skips anything else — the
  same assumption this app's `Deleted <stamp>` archive marker for deleted
  points relies on. **Build 109 found this assumption wrong for a real
  Civil3D 2024 import** — the owner uploaded a real job file this app had
  exported (with 407 hidden REF lines in it) and a screenshot of Civil3D's
  own import dialog throwing a BLOCKING "Unrecognizable input (Improper
  number of parameters)" error, with Yes/No/Cancel to abort/ignore-one/
  ignore-all, at the exact line number of one of those `-- ` markers.
  Civil3D does NOT silently skip an unrecognized line — it stops and asks,
  once per line, which would mean 407 dialogs on this real file short of
  clicking "Cancel" on the very first one. **`REF_HIDE` is no longer written
  on export at all** — a pure REF shot now exports as a perfectly ordinary
  line, identical in shape to every other point in the file (REF points are
  once again visible to CAD, the same as every build before 84; that
  cosmetic tradeoff is accepted in exchange for an import that actually
  works). Tried to find Civil3D's own documented FBK comment-line syntax
  first, to fix the marker instead of removing the feature, but every
  Autodesk domain is blocked from this sandbox (same standing limitation as
  the RT/X/RECT vendor-doc section) and no independent source states the
  real syntax — per this project's own standing rule against guessing at
  something unconfirmed, no replacement marker was invented.
- **What's kept, and why:** `REF_HIDE` the constant, and `parse()`'s
  `deHide()` step that strips a leading `REF_HIDE` before tokenizing any
  line, are both still here — purely so a job file exported by an OLDER
  build (84-108, like the one the owner uploaded) still loads correctly.
  `deHide()` still tags the resulting point `refHidden:true` when a marker
  was found, purely so `exportFBK` can find and strip a leftover one.
  `isPureRefDesc()` is gone — deleted as dead code once both of its call
  sites (the old hide-on-write and hide-in-step-3b paths) were removed.
- **`exportFBK`'s step 2d is now a one-way self-heal, not a hide/unhide
  toggle:** for every REF point, if the line it's about to export (edited or
  raw) still starts with `REF_HIDE` — left over from an older export — the
  marker is stripped and the point is logged (`refShownIds`); nothing is
  EVER re-added. Re-exporting an already-broken (build 84-108-exported) job
  file therefore fixes it automatically, with no separate "unhide" step
  needed. The step-3b end-of-file safety-net dump for standalone/COGO REF
  points (`srcLine<0`) lost its own hide branch the same way — it's now
  always a plain `NEZ` line.
- **`finalLine(p)`** (rebuilds an edited point's line — a Rod HT correction,
  a coordinate tweak) still strips a leading `REF_HIDE` before tokenizing
  (so `t[1]==='VA'` isn't shifted off by a leftover marker token, avoiding
  the same off-by-one build 84 originally fixed) but no longer restores the
  marker afterward — consistent with never writing it again.
- Verified end-to-end against the owner's OWN real uploaded file (not a
  synthetic reproduction): parsing it directly found 439 total REF points,
  407 of them flagged `refHidden:true` on load (exactly matching an
  independent `grep -c "^-- "` count on the raw file); exporting it through
  the real `exportFBK()` pipeline (the Blob intercepted directly, not a real
  download) produced **zero** lines starting with `"-- "` anywhere in the
  7502-line output; re-parsing that exported output confirmed all 439 REF
  points survive with `refHidden:false` on every one, and the figure count
  is unchanged (618) both before and after, zero console errors. A
  synthetic test separately confirmed the forward-going (never-hidden) case:
  a fresh pure `"REF"` shot exports as a plain `F1 VA ... "REF"` line with
  no marker at all, and a multi-code REF shot (`"REF RCFL1 B"`) is
  unaffected either way, exactly as before. Re-ran the complete existing
  regression battery (Knockdown/REF1/single-point-REF/orbit-zoom/snap-
  accuracy/offset-lane suites) — every one passes unaffected, since this
  change only touches how a REF line is WRITTEN on export. `index.html`'s
  inline script still parses clean (`node --check`).
- **Open item, not yet confirmed:** whether Civil3D also throws this same
  error on this app's OTHER "assume it's silently skipped" marker,
  `Deleted <stamp>` (used to archive a deleted point on export, since way
  back). The owner's screenshot only showed the FIRST error Civil3D hit
  (a `REF_HIDE` line); it's not yet known whether later dialogs for
  `Deleted` lines also appeared. If the owner reports that they do, treat
  it as a separate, real bug of the same shape — don't assume it's fine by
  analogy to REF_HIDE now being fixed.
...
## SO (stop offset) — offset span boundaries (`offsetAt`/`spanAdj`, build 66, real gap fixed build 67, false-break regression fixed build 68, mid-run value-change jog fixed build 94, per-lane lane-count changes fixed build 95, drawing-loop early-exit fixed build 96, over-eager V-only break fixed build 97, H treated the same as V build 98, a dropped lane bridges straight across instead of gapping build 99, a capped BC..EC endpoint can curve-fit build 100)
...
- **Build 94 fix — a genuine mid-run VALUE CHANGE (no `SO`) drew a diagonal
  jog instead of "old lane ends here, new lane starts here":** the owner,
  in their own words: "when new call out HV the line should draw from the
  previous call out to that point and new call out should start leaving
  from that point." Builds 66-68 correctly distinguish a real `SO` gap
  (build 66/67) from a harmless SAME-value redundant restatement (build
  68, since a curb-code shorthand re-expands to a fresh array every time
  it's re-stated even with identical numbers) — but neither one ever asked
  whether the restated value was actually DIFFERENT. A point that restates
  its own `H/V` (or switches to a different curb code) with a genuinely new
  number, with no `SO` in between, still had `cont[k]=true` (nothing ever
  stopped it) — so `offsetAt` mitered that vertex's corner using ONLY the
  NEW value on both adjacent legs, and the segment connecting back to the
  previous (OLD-value) offset point silently jogged diagonally across the
  gap between the two different offset distances instead of forming a
  clean transition. Confirmed this is a common real pattern, not a rare
  edge case: scanning the real combined bug file (the same
  `TOPO_COMBINED_-_EDITED-BUG.fbk` builds 92/93 fixed) found **113** such
  genuine value-change points.
  - `offsetPerCont` now also returns `changed[k]` — true only when
    `stepsAt[k]`'s H/V values genuinely differ (compared by VALUE, via a
    new `sameTemplate(a,b)`, never by object reference — reusing the exact
    lesson build 68 already learned the hard way) from whatever template
    was active going into vertex `k`. A same-value restatement still reads
    `changed=false` (build 68's behavior is untouched), and a resume point
    right after a real `SO` also reads `changed=false` (the template
    coming in is `null` there — nothing to differ from — so this is still
    correctly left to the pre-existing `cont`/gap mechanism, no
    double-handling of the same boundary).
  - A new shared `laneOffsetArrays(all,per,cont,changed,L)` (now used by
    both `drawOffsets` and `collectOffsetSegments`, replacing their
    previously near-duplicated per-lane loops) computes, at a
    `changed[k]&&hasPrev` vertex, TWO offset points instead of one: an
    `oldCap` — the OLD lane's own end-cap at `k`, using the PRIOR
    template's h/v with `offsetAt(...,true,false)` (a one-sided cap along
    the incoming `k-1→k` direction) — and `op[k]` itself forced to a
    NEW-lane start-cap, `offsetAt(...,false,hasNext)` (a one-sided cap
    along the outgoing `k→k+1` direction, at the NEW value). `contPrev[k]`
    is forced `false` at that vertex, so both drawing loops break there
    instead of connecting the two mismatched points directly; the old
    lane's own line gets one extra `lineTo(oldCap[k])` immediately before
    that break, so it visibly reaches vertex `k` at its own (old) offset
    distance, and the new lane picks up fresh from the same vertex at its
    own (new) distance — exactly "draw from the previous callout to that
    point, and the new callout starts leaving from that point," with no
    diagonal segment bridging the two. `collectOffsetSegments` (the
    OSNAP-facing rebuild of the same lines) got the identical treatment so
    snap targets stay consistent with what's actually drawn.
  - Verified with a synthetic 3-case check: (A) a genuine value change with
    no `SO` correctly flags `changed` at the change vertex, forces
    `contPrev` false there, and produces an `oldCap` at the OLD offset
    distance plus an `op` at the NEW offset distance with nothing
    connecting them; (B) a same-value redundant restatement stays fully
    continuous (`changed` all false, `oldCap` all null) — byte-identical to
    build 68; (C) a real `SO`-gap-then-different-value-restart is correctly
    NOT flagged `changed` (handled by the existing gap mechanism instead).
    Re-ran the complete existing Knockdown/Un-Knockdown regression battery
    (`test_kd.js`/`test_kd2.js`/`test_kd3.js`/`test_kd_lines.js`/
    `test_kd_conflict.js`/`test_kd_flip.js`/`test_kd_bulk.js`/
    `test_noside_bulk.js`/`test_unkd_bulk.js`/`test_unkd_clear_and_restore.js`)
    — every one byte-identical, since this change only touches offset
    LANE-LINE drawing, never `applyKnockdown` itself. Confirmed
    `collectOffsetSegments()` and a full `draw()` pass run with zero errors
    on both the pre-existing regression file and the real combined bug file
    (614 figures, 3087 offset segments), and screenshotted a real
    occurrence (point 7276, a genuine mid-run H-value change) before and
    after the fix to confirm the rendered output actually changed.
- **Build 95 fix — build 94's `changed[k]` was whole-template, not per-lane,
  so a callout that changes the LANE COUNT (not just a lane's value) still
  had two real gaps:** the owner sent a sketch plus a real point range
  (`7497`-`7502` on the bug file) describing it directly: "if there is call
  out making 3 offset line then next call out is making on 2 offset
  line[,] the 3rd line end[s] or wait[s] till the next call out with 3
  offset and rejoin[s] with that." Build 94's `changed[k]` fires whenever
  the WHOLE template differs (`sameTemplate` checks both length and every
  value) and every lane at that vertex was treated identically — correct
  for the simple "same lane count, one value changed" case build 94
  targeted, but wrong once a callout drops or re-adds a lane entirely (a
  real 3-step `H-1 V0.0 H-1.17 V-0.5 H-2.17 V-0.44` template shrinking to a
  2-step `H-1 V0.0 H-1.17 V-0.5` one — confirmed 9 real occurrences of
  exactly this shape in the bug file, on the `TBW`/`TTW` codes): (1) the
  DISAPPEARING lane got no end-cap at all — `laneOffsetArrays` checked
  `L<s.length` against the NEW (shorter) template first and bailed straight
  to `null`, so it just stopped one vertex too early with no clean
  perpendicular cap at the real transition point; and (2) EVERY lane —
  including the ones whose own h/v never changed — got a spurious break
  anyway, since `changed[k]` fires on a length mismatch with no per-lane
  awareness that some lanes' own numbers didn't move at all.
  - Fix: `laneOffsetArrays` no longer uses the coarse per-vertex `changed[k]`
    as its actual break decision — it now compares THIS LANE's own step
    (`prevStep` vs `curStep`, by `h`/`v` value) directly into a per-lane
    `laneChanged`, true only when this exact lane's value genuinely
    differs, or the lane appears/disappears here; `changed[k]` is kept only
    as a cheap first gate (nothing to compare at all when the whole
    template didn't change). A disappearing lane now gets its own `oldCap`
    (old-value end-cap, right at the transition vertex) even though it has
    no `op` there; a reappearing lane gets a fresh `op` start-cap with no
    `oldCap` (nothing to close, since it didn't exist a moment ago) — this
    is the literal "3rd line ends, or waits till the next callout with 3
    offset and rejoins with that": a clean stop, a real gap through the
    reduced-lane stretch, a fresh pickup once the callout restores the
    dropped lane, and the OTHER lanes staying completely unbroken the whole
    time since their own values never moved.
  - Verified with a new synthetic 3→2→3-lane repro (`test_lane_drop.js`)
    mirroring the real `TBW` shape: the persisting lanes show `contPrev`
    staying `true` throughout with zero `oldCap` entries (confirming build
    94's spurious-break bug is gone); the dropped 3rd lane draws normally
    through the 3-lane stretch, gets a real `oldCap` exactly at the drop
    point, goes `null` through the 2-lane stretch, then gets a fresh `op`
    start-cap exactly where the callout restores 3 lanes, with `contPrev`
    correctly `false` at both transitions (no jog either direction).
    Re-ran build 94's own 3-case regression (`test_offset_change.js`)
    unchanged — byte-identical, confirming the refined per-lane logic still
    produces the same result for the simple same-lane-count case build 94
    targeted. Re-ran the complete existing Knockdown/Un-Knockdown
    regression battery (`test_kd.js`/`test_kd2.js`/`test_kd3.js`/
    `test_kd_lines.js`/`test_kd_conflict.js`/`test_kd_flip.js`/
    `test_kd_bulk.js`/`test_noside_bulk.js`/`test_unkd_bulk.js`/
    `test_unkd_clear_and_restore.js`) — every one byte-identical. Confirmed
    `collectOffsetSegments()`/a full `draw()` pass still run with zero
    errors and unchanged figure/segment counts on both `job.fbk` and the
    real combined bug file; found and dumped 9 real lane-count-change
    points in the bug file (`TBW`/`TTW` codes) confirming this is a real,
    not just synthetic, pattern.
- **Build 96 fix — build 95 computed the right data but neither drawing
  loop ever actually rendered it:** the owner uploaded the real job file
  (`TOPO LEXINGTON-EDITED.fbk`) their original sketch and point range
  (`7497`-`7502`) actually came from and reported "It not behaving like I
  showd you." `laneOffsetArrays` (build 95) correctly computes `oldCap[k]`
  — the disappearing lane's own old-value end-cap — at a vertex where a
  callout drops a lane. But both places that actually consume that data,
  `drawOffsets`'s lane-line loop and `collectOffsetSegments`, checked
  `op[k]` FIRST (`if(!op[k]){started=false;...}` / `if(!o){prev=null;
  return;}`) and bailed out before ever reaching the `oldCap[k]` check
  below it. At exactly the vertex where a lane disappears, `op[k]` IS
  correctly `null` (no new point for that lane there) — which is exactly
  the vertex whose `oldCap[k]` needed to be drawn — so the early exit
  skipped past it every single time. Net effect: the disappearing lane
  simply stopped one real vertex too early with no closing segment
  drawn at all, instead of the clean end-cap the owner's sketch showed —
  a real rendering bug build 95's own unit tests never caught, since they
  only checked `laneOffsetArrays`'s return values, never that the drawing
  loops actually consumed `oldCap` correctly.
  - Confirmed directly against the owner's real Lexington file: point
    7497 (`"RBCB1 H-1 V0.0 H-1.17 V-0.5"`) drops from 7443's 3-lane
    `H-1/H-1.17/H-2.17` template to just 2 — exactly the reported case.
    `oldCap` for the dropped 3rd lane computed correctly at 7497
    (`{E:561270.697,N:159061.145}`, ~2.17 ft from the base point, matching
    the dropped `H-2.17` lane's own magnitude) but was never rendered.
  - Fix: moved the `if(started&&oldCap[k]){...lineTo...}` check in
    `drawOffsets` to run BEFORE the `if(!op[k]){started=false;...}` early
    exit, so it fires even when this lane has no new point at `k`; made
    the identical reordering in `collectOffsetSegments` (`if(prev&&
    oldCap[k])` now runs before `if(!o){prev=null;return;}`). Both changes
    are pure reordering — no new conditions, no changed math — so every
    already-correct case (same-lane-count value changes, redundant
    same-value restatements, real `SO` gaps) is untouched; only the
    "lane disappears entirely" case, which previously could never reach
    the `oldCap` branch at all, now does.
  - Verified against the owner's real file: a screenshot centered on 7497
    now shows the 3rd green offset line ending in a clean, isolated
    segment stopping exactly at the transition point (previously it just
    vanished one vertex early with no visible end); a screenshot at 7502
    shows the 3rd lane starting fresh there, no diagonal connecting back
    across the 2-lane gap — both match the owner's own sketch shape-for-
    shape. `collectOffsetSegments()` re-checked directly: it now emits a
    segment ending exactly at the same `{561270.70,159061.15}` point the
    draw path uses, confirming OSNAP and the visible canvas agree. Re-ran
    build 95's own synthetic 3→2→3-lane test (`test_lane_drop.js`) and
    build 94's 3-case test (`test_offset_change.js`) — both byte-identical
    (this fix is in the DRAW consumers, not the data-producing
    `laneOffsetArrays`, so their outputs are unaffected). Re-ran the
    complete existing Knockdown/Un-Knockdown regression battery
    (`test_kd.js`/`test_kd2.js`/`test_kd3.js`/`test_kd_lines.js`/
    `test_kd_conflict.js`/`test_kd_flip.js`/`test_kd_bulk.js`/
    `test_noside_bulk.js`/`test_unkd_bulk.js`/`test_unkd_clear_and_restore.js`)
    — every one byte-identical. Confirmed `collectOffsetSegments()`/a full
    `draw()` pass still run with zero errors on both `job.fbk` and the
    combined bug file; offset-segment counts rose slightly on both
    (job.fbk 3033→3077, bug file 3087→3090) — expected and correct, since
    these are exactly the previously-missing closing segments this fix
    now draws, not a sign of anything spurious.
- **Build 97 fix — builds 94-96's own "genuine value change" detection was
  over-firing on ordinary V-only variation, breaking lines that should have
  flowed:** the owner, right after confirming build 96: "the offset [lines]
  are showing breaking and not flowing to the transition like [a] build
  ago," clarified as happening "on regular curb code and when applying the
  ref vertical adjustment." Builds 94-96's `laneChanged` check compared a
  lane's `h` AND `v` for exact equality — correct for a genuine new distance
  callout (build 94's own target case), but wrong for the far more common
  case of a V-only difference: a real curb's own vertical reveal legitimately
  varies shot to shot along one continuous run at the SAME offset distance,
  whether it's ordinary crew-typed `H<v> V<v>` text (confirmed on the real
  combined bug file, point 3916→3917: `H0.5 V0.0 H0.67 V-0.5 H2.67 V-0.38` →
  `H0.5 V0.0 H0.67 V-0.02 H2.67 V0.1` — same 3 H values, V drifting up to
  0.48 ft) or, even more so, Knockdown's own **REF-derived reveal**
  (`refKdTmpl`, see the Knockdown section above) — which by design computes
  a genuinely unique V from each point's own nearby REF geometry, so two
  adjacent REF-baked points on the very same curb code will almost always
  differ in V even though nothing about the callout itself changed. A
  real-Knockdown sweep of the combined bug file found **231 of 345 (67%)**
  of every `changed[k]`-flagged transition were exactly this same-H/
  different-V shape, every one of them getting a spurious break instead of
  flowing through. Fix: `laneOffsetArrays`'s `laneChanged` now compares
  ONLY `h`, dropping the `v` half of the equality entirely — H is which
  physical offset distance/lane this is (a real H change still breaks,
  unchanged); V is just that lane's own vertical reveal at one point, and
  `offsetAt` already only ever uses THIS vertex's own `h`/`v` (never the
  previous point's) to compute the miter, so letting a same-H/different-V
  vertex flow through the ordinary continuous branch produces exactly the
  right thing — same offset distance, normal corner mitering, Z simply
  following the new V, the same way the base line itself already flows
  through varying real Z without breaking. `changed[k]`/`sameTemplate` (the
  coarse whole-template gate) were left untouched; a lane appearing/
  disappearing (build 95) is unaffected, since that's decided by
  `prevStep`/`curStep` being `null`, not by this comparison. Verified: of
  every `changed[k]` event in the real combined bug file, **732 same-H
  lane-vertex instances now correctly flow continuously, zero exceptions;
  235 genuinely-different-H instances still correctly break, zero
  exceptions** — a perfect split. Re-ran build 96's own real Lexington
  7497→7502 lane-drop verification unchanged (still correct — a presence
  check, not a value comparison), build 94's 3-case test and build 95's
  3→2→3-lane test both byte-identical (neither synthetic scenario happened
  to be a same-H/different-V case), and the complete Knockdown/Un-Knockdown
  regression battery byte-identical. `collectOffsetSegments()`/a full
  `draw()` pass and a real Knockdown-apply pass still run with zero errors
  on `job.fbk` (612 figures), the combined bug file (614 figures), and the
  real Lexington file (114 figures). `index.html`'s inline script still
  parses clean (`node --check`).
- **Build 98 fix — H needed the exact same treatment as V:** the owner,
  right after confirming build 97: "That's better. But you should treat
  the horizontal the same way you did the vertical. Let's try this again."
  Build 97 stopped a same-H/different-V restatement from breaking, but a
  same-V/different-H restatement (or one where BOTH differ) still forced a
  break — the same reasoning that justified letting V flow (a real curb's
  own number legitimately varies point to point along one continuous run,
  and isn't "a new lane") applies just as directly to H: a real offset
  distance can genuinely widen or narrow gradually shot to shot (a
  driveway apron, a widening walk) without being a brand-new, disconnected
  line, and `offsetAt` already mitres purely off THIS vertex's own `h`/`v`
  and its real neighboring points' directions — never the previous point's
  `h` — so a changing H flows through the same ordinary continuous branch
  build 97 already proved safe for V, tracking the new distance smoothly
  instead of jogging to a disconnected fresh start. Fix:
  `laneOffsetArrays`'s `laneChanged` dropped the `prevStep.h===curStep.h`
  comparison entirely — `!(prevStep&&curStep)` — so a lane now only breaks
  when it genuinely **appears or disappears** (one side has no step for
  this lane at all, build 95's lane-count-change case); as long as the
  lane exists on both sides, it flows continuously regardless of what its
  own H or V numbers do. Verified against the real combined bug file:
  **977 lane-vertex instances where the lane exists on both sides now flow
  continuously, zero exceptions** (up from build 97's 732, now also
  covering the 235 different-H cases build 97 still broke); **10 genuine
  appear/disappear instances still correctly break, zero exceptions**.
  Re-ran build 96's own real Lexington 7497→7502 lane-drop verification
  unchanged (byte-identical — a presence check, untouched by this value-
  comparison relaxation) and build 94's own 3-case test: case A (a
  synthetic `h:-1→h:-2` genuine distance change, previously the very
  example that justified breaking) now correctly flows (`A_contPrev=
  [false,true,true,true]`, `A_oldCap` all null) — the deliberate, requested
  behavior change; cases B/C unaffected. Re-ran the complete existing
  Knockdown/Un-Knockdown regression battery plus `test_lane_drop.js` — all
  exit clean, since this change is scoped entirely to offset lane-line
  continuity, never `applyKnockdown` itself. Confirmed
  `collectOffsetSegments()`/a full `draw()` pass and a real Knockdown-apply
  pass still run with zero errors on `job.fbk` (612 figures), the combined
  bug file (614 figures), and the real Lexington file (114 figures),
  figure counts unchanged from build 97. `index.html`'s inline script
  still parses clean (`node --check`).
- **Build 99 — a dropped lane now bridges STRAIGHT across to wherever it
  next reappears, instead of a clean end-cap and a real visible gap:** the
  owner, with a concrete real example: "example 7497 to 7502 on the
  Lexington file... make it there the 3rd out most line piot 7497 connect
  to 7502 sipping 7498,7499,7500." Builds 95/96 deliberately gave a
  dropped lane a perpendicular end-cap, a real gap through the
  reduced-lane stretch, and a fresh start-cap where the callout restored
  it (the owner's own build-95 wording, "the 3rd line ends, or waits till
  the next callout... and rejoins with that"). This build asks for
  something different: no gap — the outermost lane should run straight
  from its last real point to its next one, skipping whatever's in
  between, the way a real curb line wouldn't show a hole just because a
  callout omitted that lane for a few shots. `laneOffsetArrays` now tracks
  `lastStep` (this lane's own most recently seen value, cleared only by a
  real SO or the start of the run) — a vertex with no step of its own but
  a live `lastStep` gets a one-sided cap placed AT that real vertex using
  the OLD value (tagged `real:false`, so the line still visibly reaches
  it), and every vertex deeper into the drop is left `null`. Both drawing
  consumers now treat a `null` as a **soft skip** — `started`/`prev` only
  resets when `cont[k]` says an actual `SO` broke it there; otherwise they
  skip straight through, so when the lane's value resumes, a plain
  `lineTo` bridges directly from the cap to the new point, cutting across
  the gap with no jog, no moveTo, no visible break. Build 94-98's
  `changed[]`/`oldCap` machinery (the "compare values, break on any
  change" model) is gone — `laneOffsetArrays` now only distinguishes "does
  this lane have a step here" from "did an SO actually stop it" (still
  purely `cont[]`-driven, unchanged); `offsetPerCont`'s `sameTemplate`/
  `changed[]` were dead code once nothing consumed them and were deleted.
  **A real bug caught while implementing this:** the BC..EC curve-fitting
  branch gated on `op.slice(k-1,j+1).every(Boolean)` to check every
  vertex in a span had a real per-vertex h/v — with drop-caps now able to
  make `op[k]` non-null even when `per[k]` doesn't carry the lane, that
  check would read a cap as a genuine step and crash reading
  `per[m][L].h` on a vertex with no such entry; added a `real[]` array
  (true only for a genuine step) and switched that check to
  `real.slice(k-1,j+1).every(Boolean)` — caught by a real crash in
  headless testing first. Verified: the exact reported Lexington case
  (`RBCB1`, points 7493-7507) now shows lane 2's `op` as
  `[…,7497:CAP(E=561270.697,N=159061.145 — the same value build 96
  already ground-truthed),7498:null,7499:null,7500:null,7502:{…,
  contPrev:true},…]` — exactly "7497 connects to 7502, skipping
  7498/7499/7500," zero console errors. A new whole-file invariant check
  (`test_build99_verify.js`) confirms across `job.fbk` (3433 drawable
  lane-points), `bugfile.fbk` (3384), and `lexington.fbk` (783):
  **`contPrev[k]` equals `cont[k]` at every single drawable point, zero
  exceptions** — a gap can now ONLY come from a genuine SO, never from a
  value change or a lane merely dropping in and out — and real bridged
  gaps are genuinely exercised on real data (17/1/26 respectively on the
  3 files), not just the synthetic case. Re-ran the complete existing
  Knockdown/Un-Knockdown regression battery — all pass, unaffected.
  `test_lane_drop.js`/`test_offset_change.js` (builds 95/94's own
  synthetic scenarios) were updated in place to the new
  `{op,contPrev,real}` shape and the new bridging expectations — the only
  intentional assertion changes, matching exactly what this build
  changes; `test_fix98_verify.js`'s build-98-specific "appear/disappear
  always breaks" assertion is superseded outright by `test_build99_verify.js`.
  Confirmed `collectOffsetSegments()`/a full `draw()` pass and a real
  Knockdown-apply pass still run with zero errors on `job.fbk` (612
  figures), the combined bug file (614 figures), and the real Lexington
  file (114 figures). `index.html`'s inline script still parses clean
  (`node --check`).
- **Build 100 — a capped BC..EC endpoint couldn't curve-fit, so a real
  curved span stopped curving the moment its OWN callout dropped a lane
  right at the curve's `EC`:** the owner, immediately after confirming
  build 99: "next I want you to fix that outer most line is not curving
  7502 to 7512 on Lexington file." Root-caused directly against the real
  file: point 7502 (`"RBCB1 BC H-1 V0.0 H-1.17 V-0.5 H-2.17 V-0.44"`, 3
  lanes) starts a genuine curved run through 7504/7507/7508/7510 to its
  `EC` at 7512 — but 7512's own callout (`"RBCB1 EC H-1 V0.0 H-1.17
  V-0.5"`) only restates 2 lanes, not 3 (a real, ordinary EC callout that
  simply doesn't re-state the outermost lane's value right at the curve's
  own end). For lane 2 (the outermost), that's exactly build 99's "lane
  drop" case — `laneOffsetArrays` correctly gives 7512 a one-sided **cap**
  there (borrowing lane 2's last real value from 7510), tagged `real:
  false` since it's a borrowed value, not a fresh one. But `drawOffsets`'s
  BC..EC curve-fit eligibility check, `real.slice(k-1,j+1).every(Boolean)`
  (added in build 99 specifically to avoid sampling a curve through a true
  gap), couldn't tell "a perfectly good capped position" apart from "no
  position at all" — it saw 7512's `real:false` and rejected the WHOLE
  7502-7512 span, falling through to the plain per-vertex straight
  `lineTo` for that entire lane. Confirmed directly: dumping `lane2_real`
  across the real span read `true` at every vertex except `false` at
  exactly 7512 (the EC point) — matching "not curving" precisely.
  Fix: `laneOffsetArrays` now also returns `hv[]` — the `{h,v}` that
  actually produced `op[k]`, whether from a genuine step or a cap's
  borrowed value — and the eligibility check switched from
  `real.slice(...).every(Boolean)` to `op.slice(...).every(Boolean)` (any
  drawable point, real or capped, is a valid curve endpoint; only a true
  `null` gap still disqualifies the span), with the curve fit's `hSub`
  built from `hv[m].h` instead of reaching into `per[m][L].h` directly —
  which sidesteps the exact crash build 99 originally guarded against
  (`per[m]` has no entry at index `L` for a capped vertex) by carrying the
  effective h/v alongside `op` instead of re-deriving it from `per`.
  Verified against the real Lexington file: the eligibility check for
  lane 2's 7502-7512 span now reads `true` (was `false`), `curvedOffsetPts`
  returns 40 smooth curve samples along the real fitted arc (was 0,
  falling to the straight per-point path), and a real headless-browser
  screenshot centered on the 7502-7512 hairpin shows all 3 green offset
  lanes curving smoothly and concentrically with the magenta base line
  through the whole turn, matching the base curve's shape with no facet.
  Re-ran the complete existing regression battery (`test_kd.js`/
  `test_kd2.js`/`test_kd3.js`/`test_kd_lines.js`/`test_kd_conflict.js`/
  `test_kd_flip.js`/`test_kd_bulk.js`/`test_noside_bulk.js`/
  `test_unkd_bulk.js`/`test_unkd_clear_and_restore.js`/
  `test_offset_change.js`/`test_lane_drop.js`/`test_build99_verify.js`) —
  every one still passes unchanged, including `test_build99_verify.js`'s
  own `contPrev[k]===cont[k]` invariant (zero violations, same 26 real
  bridged gaps on the Lexington file) — confirming this fix only widens
  which points a curve-fit may sample from, without touching the SO/gap/
  bridging behavior those tests check. `index.html`'s inline script still
  parses clean (`node --check`).
...
## Export DXF (`exportDXF`, ↓ Export DXF button, build 111)

- Writes a drawing, not just data — FBK/CSV export gave text a downstream CAD
  package could import, but never a drawing file with the actual linework
  already traced. `exportDXF()` produces a plain ASCII (R12-style) DXF with
  just an `ENTITIES` section — no `HEADER`/`TABLES` needed, since AutoCAD/
  Civil3D auto-creates any layer an entity references that doesn't already
  exist in the drawing, the same shortcut every minimal DXF writer relies on.
- **Coordinate convention:** `X=Easting, Y=Northing, Z=Elevation` throughout —
  the standard state-plane CAD convention, matching how a PNEZD point file
  (Export CSV) already gets consumed downstream.
- **What gets written, and from where:**
  - Every non-deleted point → a `POINT` entity (layer `POINTS`, or
    `CONTROL-POINTS` for `p.kind==='ctrl'`) plus a `TEXT` label using
    `pointLabelText(p)` — the exact same string the 🏷 Labels toolbox already
    shows on screen (falls back to the bare point id if every label checkbox
    is off, so a point is never unlabeled in the DXF).
  - Every figure (`figures()`) → `figurePolyline(f)`, the SAME
    already-verified (build 107) function rendering and the snap engine both
    use for a figure's actual tessellated geometry (a circle's full loop,
    each `BC..EC` sub-span's fitted arc/spline, an `OC` tangent arc, every
    `RT`/`X`/`RECT` jog) — never a re-derived approximation. A circle figure
    becomes a true `CIRCLE` entity; anything else becomes an old-style 3D
    `POLYLINE`/`VERTEX`/`SEQEND` block (real per-vertex Z survives, so a
    curve's elevation profile comes through, not just its plan shape) on a
    layer named `LINE-<code>` (`dxfLayer()` sanitizes the code to valid DXF
    layer characters).
  - CPN/RPN ties → `LINE` entities on layer `TIES`, from both sources a tie
    can come from: a figure's own `f.links`, and any tie whose origin point
    isn't part of a figure at all (the same `CONNECT`-minus-figured-indices
    logic `drawConnectLinks` already uses, so nothing gets drawn twice and
    nothing gets missed).
  - Standalone `⊙ CTR` circles (the `circles` global) → `CIRCLE` entities on
    layer `CIRCLES`, at the same `radiusFt*0.3048` world radius `drawCircles`
    itself renders at.
  - Curb offset cross-section lane lines → `POLYLINE`s on layer
    `OFFSET-<code>`, built by calling `offsetPerCont`/`offsetLaneChains`
    *directly* — the exact functions `drawOffsets`/`collectOffsetSegments`
    already use, carrying forward this project's entire build 94-100/108
    gap/bridge/curve-continuity history for free — rather than re-deriving
    that geometry a third time and risking a fresh instance of the same
    class of subtle bug this codebase has spent 15+ builds fixing there.
- **Nothing new was invented for geometry** — every entity's shape comes from
  a function this app already renders and snaps with. The DXF writer's own
  code (`dxfLayer`/`dxfNum`/`dxfPoint`/`dxfText`/`dxfLine`/`dxfCircle`/
  `dxfPolyline`) is pure text formatting, nothing that computes a curve,
  offset, or intersection itself.
- Verified end-to-end in a real headless browser against a synthetic job
  exercising every entity type at once (a plain 2-vertex line, a `BC..EC`
  curve, a lone 3-shot `CIR` cluster, and an `RBCB1` curb run with a real
  `H/V` offset template through its own `BC..EC` span): the DXF starts with
  `SECTION`/`ENTITIES` and ends with `ENDSEC`/`EOF`; exactly one `POINT` and
  one `TEXT` per parsed point; the `CIR` cluster produces a `CIRCLE`; the
  straight line and the curve both produce a `POLYLINE` with a matching
  `SEQEND`, the curve's with more than 2 `VERTEX` records (confirming real
  tessellation, not a straight chord); the `RBCB1` run's `H/V` template
  produces a real offset-lane chain that shows up as an `OFFSET-RBCB1`
  polyline; zero `NaN`/`undefined` anywhere in the output; zero console
  errors beyond the pre-existing, already-documented map-tile network block.
  A second synthetic test confirmed a `CPN` tie between two figures exports
  as a `LINE` entity on the `TIES` layer. `index.html`'s inline script still
  parses clean (`node --check`).
...
- Basic CAD object snap (`snapPoint`, 2D only) in priority order: **endpoint**
  (any point/vertex — **and, since build 107, a circle's own center and its 4
  quadrant points**, from `collectCircles()`), **intersection** of two lines
  (since build 107 this requires the two REAL segments to genuinely cross —
  see below; before that it was "apparent," lines extended past their own ends
  until they crossed), **midpoint**, then **nearest** on a segment. Z is
  interpolated along the segment (W2S is affine, so the screen-space parameter
  equals the world parameter); an intersection's Z is the average of the two
  lines' Z at the crossing.
- **Build 107 — snap follows the actual rendered line, not a straight
  vertex-chord approximation of it.** `collectSegments()` used to build one
  straight segment per pair of consecutive FIGURE VERTICES, so a `BC..EC`
  curve, an `OC` tangent-arc corner, and an `RT`/`X`/`RECT` jog chain — all of
  which draw through extra COMPUTED points, not just the real shots — snapped
  onto their straight chord instead of the actual curve, sometimes tens of
  feet off on a real compound curve. Fixed by extracting the exact point-walk
  `strokeFigureMain` already used to draw a figure into a shared
  `figurePolylineFromVerts(f,vs)`/`figurePolyline(f)` (a circle's full
  72-sample loop, each curve sub-span's fitted samples, a tangent arc's
  samples, every jog point) — `strokeFigureMain` now just draws whatever this
  returns, so rendering and snapping read from the exact same geometry, with
  no way for the two to drift apart. `collectSegments()` also now includes a
  figure's own CPN/RPN tie lines, any CPN/RPN tie whose origin isn't part of a
  figure at all, and every standalone `⊙ CTR` circle — none of these could be
  snapped onto at all before. The curb offset lane lines
  (`collectOffsetSegments`) were LEFT UNCHANGED in this build — still straight
  per-vertex chords, a known pre-existing tradeoff (see build 27 below)
  deliberately left alone given `drawOffsets`'s own long, fragile bug history
  (builds 66-100). **Build 108 fixed this** — see below; the owner reported
  "snap not working on offset lines" right after this build shipped, and it
  turned out to be exactly this gap (a curved offset lane's chord could sit
  hundreds of pixels from the actual rendered curve).
- **Build 107 — intersection now requires the two segments to actually
  cross.** The owner asked for this explicitly, reversing build 12's own
  original "apparent" (extended-lines) design. `lineLineX`'s `t`/`u`
  parameters are now both checked against `[-0.02,1.02]` (a small numerical
  margin, not an open-ended extension) before a crossing counts — in both
  `snapPoint` and `pickSegmentForInsert` (which previously bounded only its
  OWN line's `t`, never the other line's `u`). Every segment also carries a
  `src` tag identifying which single rendered line it came from (a figure id,
  `id|off<lane>` for an offset lane, `id|link|<pt>`/`connect|<a>|<b>` for a
  tie, `circle|<i>` for a standalone circle); the intersection scan skips any
  pair sharing the same `src`, since two adjacent samples of ONE figure's own
  fine curve tessellation trivially "intersect" at the sample point they
  share — without this, the finer segments from the point above would
  manufacture a false self-intersection on every curve on screen. `drawSnapMarker`'s
  label/comments were reworded from "apparent intersection" to plain
  "intersection" throughout.
- **Build 108 — the curb offset lane lines now snap onto their actual
  rendered geometry too (straight, curved, or the OC tangent arc), not just
  a straight chord across the whole lane.** The owner's report right after
  build 107 shipped — "SNAP NOT WORKING ON OFFSET LINES" — traced to exactly
  the tradeoff build 107 disclosed and deliberately left alone: a `BC..EC`
  or `OC` offset lane's own chord (straight point-to-point across the whole
  curved/rounded span) could sit **hundreds of pixels** from the actual
  drawn curve, so hovering anywhere near the visible (curved) green offset
  line found nothing within any snap tolerance — confirmed numerically (a
  244px gap on a synthetic curved lane at a typical zoom). A plain straight
  offset lane snapped fine both before and after, which is why the report
  read as a blanket failure rather than "only broken on curves." Fixed by
  extracting `drawOffsets`'s own per-lane point-walk — straight runs,
  `curvedOffsetPts` through a `BC..EC` span, `offsetTangentArc` through an
  `OC` corner, and build 99's gap/bridge/cap rules for a lane a callout
  temporarily drops — into a shared `offsetLaneChains(all,per,cont,bcAt,
  ecAt,ocAt,L)` that returns the lane's real drawn geometry as world-point
  chains (breaking only at a genuine `SO`, exactly like the draw loop)
  instead of calling `ctx.moveTo`/`lineTo` directly. `drawOffsets` now just
  draws whatever this returns (byte-verified unchanged rendering);
  `collectOffsetSegments()` walks the SAME chains into fine snap segments
  instead of one chord per lane — the identical single-source-of-truth
  pattern build 107 already used for `figurePolyline`, just extended to the
  one function build 107 had explicitly left out. `offsetPerCont`/
  `laneOffsetArrays` themselves (the fragile build 94-100 machinery) were
  not touched — only how their output becomes drawable/snappable shape
  changed. See the build 108 entry in the version-history table above for
  full verification (render-output byte-identical, snap-accuracy numbers,
  and the full regression battery).
- A live osnap marker is drawn under the cursor (▢ endpoint, ○ intersection,
  ...
  circle per the owner's request; endpoint/midpoint already matched; build 107
  added a circle+cross for **center** and a diamond for **quadrant**, both
  folded under the same `endpoint` OSNAP toggle rather than a new checkbox).
  No snap in range → free pick (N/E only, Z left for the user). **Esc**
  cancels and reopens the dialog. Both the figure linework (build 107: the
  actual rendered curve/circle/jog geometry, not a straight-chord
  approximation — see above) **and the curb cross-section offset lane lines**
  are snapped (`collectOffsetSegments` rebuilds the offsets as world segments
  from `offsetLaneChains` — the same actual drawn geometry `drawOffsets`
  itself uses, straight/curved/tangent-arc alike, since build 108 — see the
  build 108 note above).
...
### Entering orbit with a point selected zooms to it (`setMode`, build 104)

- The owner: "when I have point select and I press to 3d orbit the[n] that
  default to zoomed to point and when nothing is select it behavior like
  normal like before."
- `setMode('orbit')` — the single function both the `#tOrbit` toolbar button
  and the `O` keyboard shortcut call — already ran `fit()` on the 2D→3D
  transition, unconditionally framing the WHOLE model regardless of any
  current selection. If a point (`sel>=0`, not deleted) is selected at that
  exact moment, `fit()` still runs first (for a sane model-scale baseline),
  then the pivot is re-targeted straight onto that point's own `E/N/Z`
  (`orbit.cx/cy/cz`, the same "pivot = a real point" idea build 80's
  `retargetOrbitPivot` already established for orbit-dragging, just applied
  once up front here), the zoom floor is bumped to `orbit.s=Math.max(orbit.s,
  6)` (the identical `6` px-per-foot floor `zoomToPoint`'s 2D branch already
  uses — `orbit.s` scales world coordinates the same way `view.s` does, so
  the same constant carries straight over), and `ox/oy` are zeroed — with
  the pivot now equal to the point's own coordinates, `P3` projects it to
  exact screen-center automatically. The same gold flash-ring
  (`flash={i:sel,...}`) build 41's `zoomToPoint` already uses plays for
  feedback.
- **Nothing selected → completely unchanged:** `sel<0` skips straight to the
  same plain `fit()` every orbit-entry already used, framing the whole
  model exactly as before this build. A selected point that's since been
  deleted falls back the same way (mirrors `zoomToPoint`'s own guard).
  Re-clicking Orbit while already in 3D (no view transition happening)
  still only ever hits the untouched `else draw()` branch, so it never
  re-snaps the view while already orbiting.
- Verified end-to-end through the real, running UI in a real headless
  browser with synthetic points spanning a large area (no sample `.fbk`
  checked into this repo): no selection reproduces `fit()`'s own
  whole-model centroid pivot byte-for-byte; a real, non-centroid selected
  point (via an actual `#tOrbit` click) lands the pivot exactly on that
  point's `E/N/Z`, `orbit.s` at the `6` floor, and the point's `P3()`
  projection at exact screen-center; re-clicking Orbit in 3D leaves the
  pivot untouched; a selected-then-deleted point correctly falls back to
  the whole-model fit. `index.html`'s inline script still parses clean
  (`node --check`).
...
## Face-2 (reciprocal-face) shots — export was silently un-reversing them (`finalLine`, build 110)

- **What "Face 2" means here:** a total-station shot recorded on the reversed
  telescope face reads its zenith angle as the explement of the true angle
  (`za_raw = 360 − za_true`) and its horizontal angle offset by 180° from the
  Face-1 equivalent. `parse()` (line ~743) and `applyRawLine()` (the raw-FBK
  code-editor path) both detect this and correct it the moment a line is
  read: `if(t[0]==='F2'&&za>180){za=360-za;ha=(ha+180)%360;}`. From that
  point on, **`p.ha`/`p.za` are ALWAYS stored in the corrected
  (Face-1-equivalent) form** — the raw, as-shot Face-2 numbers are never kept
  anywhere on the point object. The Inspector's own HA/ZA edit fields
  correctly show this same corrected value (that's the right thing to let
  someone edit) — this was never the bug.
- **The bug:** `finalLine()` — the one function behind both the "Review/edit
  FBK code" line preview and the real file export — took a shortcut for
  every `F1`/`F2 VA` line that ignored all of this: it always wrote
  `t[3]=deg2dms(p.ha)` and `t[5]=deg2dms(p.za)` straight from the
  already-corrected internal values, while leaving `t[0]` (the original
  `F1`/`F2` token, read straight off the source line) completely alone. A
  Face-1 shot is unaffected by this (F1 never gets corrected, so `p.ha`/
  `p.za` already equal the raw values) — but a genuine Face-2 shot whose raw
  ZA was over 180° got its parse-time correction silently **undone** on
  every export: the corrected (now ≤180°) angle got written back out under
  the unchanged `F2` label, which is exactly the "HA shifted 180°, ZA =
  360−ZA vs. the original" signature a QC diff between two exports of the
  same job would find — and did: the owner had an independent QC pass done
  comparing `TOPO_PASCAL-EDITED.fbk` against a combined
  `TOPO_COMBINED_-_EDITED-SEP15.fbk`, which flagged 7 points
  (11331/11399/11428/11461/11465/11467/11471) with exactly this reciprocal
  transformation between the two files, both still labeled `F2 VA` in both.
  This wasn't limited to those 7 points — it happens on **every** untouched
  Face-2 shot in **any** file this app exports; those 7 were just whichever
  ones the manual diff happened to check.
- **Why this matters even though re-loading the app's own export back into
  the app itself still computes the right N/E/Z either way** (the
  correction is its own inverse, and the app's `za>180` gate never
  double-fires on its own re-parse of its own already-corrected output) —
  the real risk is any OTHER reader of the exported file (Civil3D, the
  original data collector, or a QC diff exactly like the one that caught
  this) that expects `F2` to still mean "needs the standard correction." A
  tool that gates on ZA>180° the same way this app does would see an
  already-corrected line and correctly do nothing — fine. A tool that
  flips any `F2` line unconditionally would undo the app's own correction
  and land back on the original, WRONG pre-correction angle — a silent,
  genuine position error introduced purely by passing the file through
  this app's export.
- **Fix:** `finalLine()` now re-derives, fresh from the untouched
  `RAW[p.srcLine]` text on every call (never from a stored per-point flag),
  whether THIS line's own original face + ZA combination would have
  triggered the parse-time flip — `const flipped=t[0]==='F2'&&
  dms2deg(+t[5])>180` — a fixed property of the source line's own original
  text, completely independent of whether the point has since been edited —
  and un-applies that same transformation to the CURRENT `p.ha`/`p.za`
  before serializing: `outHa=flipped?(p.ha+180)%360:p.ha;
  outZa=flipped?(360-p.za)%360:p.za`. An untouched point's corrected value
  un-flips right back to its original raw appearance (numerically exact,
  modulo the same DMS-string round-trip precision `id`/`sd` already had); a
  genuinely edited Face-2 point (a rod-height fix, a coordinate edit, or a
  direct angle edit in the Inspector) still serializes correctly — the
  edit lands in the SAME raw-Face-2-looking convention (ZA still >180°) the
  source line used, so it reduces to the right position whether re-parsed
  by this app or by any other tool following the standard reciprocal-face
  convention.
- Verified against the REAL extracted `finalLine`/`dms2deg`/`deg2dms`
  source (not a reimplementation), first reproducing the exact bug
  numerically (a raw `F2 VA` shot with HA 45°/ZA 250.5432° parses to the
  corrected 225°/110°, and the OLD code exported it right back out as
  `F2 VA ... 225 ... 110` — the identical transformation the QC report
  described) before writing the fix, then confirming 4 cases against the
  fixed code: an untouched genuine Face-2 shot (raw ZA>180°) now round-trips
  its HA/ZA back to the original raw values, where the old code exported
  the wrong, reversed pair; a plain `F1` shot is completely unaffected; an
  `F2`-labeled shot whose raw ZA was already ≤180° (never needed the flip
  in the first place) round-trips unchanged either way; and a genuinely
  EDITED Face-2 shot (Inspector ZA changed from the corrected 110° to
  105°) exports as a proper raw-Face-2-looking line (ZA still >180°) that
  re-parses back to exactly the edited 105°, not the original 110° and not
  a double-flip.
- **Checked every other area an angle-on-a-shot could get "messed up" the
  same way** (per the owner's ask to look everywhere this can happen), by
  grepping every `deg2dms(p.ha)`/`deg2dms(p.za)` call site in the file:
  the Inspector's own HA/ZA edit fields (correctly show the corrected
  value, by design — this was never wrong) and the `moveTo`/edit-log
  message (a cosmetic before→after string in the edit log, never exported)
  were both already fine and needed no change. `finalLine()` was the only
  place with this bug. `index.html`'s inline script still parses clean
  (`node --check`).
