...
- **Build 107 — snap follows the actual rendered line, not a straight
  ...
  (`collectOffsetSegments`) are UNCHANGED — still straight per-vertex chords,
  a known pre-existing tradeoff (see build 27 below) deliberately left alone
  given `drawOffsets`'s own long, fragile bug history (builds 66-100).
...
- A live osnap marker is drawn under the cursor (▢ endpoint, ○ intersection,
  ...
  using the same math as `drawOffsets`; still straight per-vertex chords,
  see the build 107 note above).
