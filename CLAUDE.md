...
## Knockdown behavior (⚙ button → `applyKnockdown()`, per-marker-segment + REF-vs-Standard ask build 86, per-marker-key code fallback build 88, forward-vs-nearest L/R conflict flagging build 89, REF-default + bulk-select review build 90, unknown-side ask build 92 REMOVED build 103 — see below, control-token bake ordering build 101, multi-code segment isolation build 102)
