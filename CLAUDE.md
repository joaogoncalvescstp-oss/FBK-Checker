...
## Knockdown behavior (⚙ button → `applyKnockdown()`, per-marker-segment + REF-vs-Standard ask build 86, per-marker-key code fallback build 88, forward-vs-nearest L/R conflict flagging build 89)
...
- **RBCB** (rod on back of curb) → uses the **Back-of-Curb DB** (`CURB_BOC`).
  - A segment with its own curb code AND a REF point within range (the
    ...
    a `Standard` (pre-checked, matching the old default) or `REF <id>` radio
    choice, computed v-value and distance shown for the REF option. Every
    OTHER, unambiguous point/segment in the same Knockdown run still bakes
    automatically the moment **Apply** is hit, alongside whichever choice was
    made for the ambiguous ones. **Cancel** aborts the WHOLE run — nothing is
    written until Apply, same contract as the UGW→CPN review popup.
