...
## SO (stop offset) — offset span boundaries (`offsetAt`/`spanAdj`, build 66, real gap fixed build 67, false-break regression fixed build 68, mid-run value-change jog fixed build 94, per-lane lane-count changes fixed build 95, drawing-loop early-exit fixed build 96, over-eager V-only break fixed build 97)
